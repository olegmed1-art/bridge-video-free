...
- `0200–0299`: **School Autopilot / durable workflow orchestration** only. New files in this range must start with `autopilot_` or `workflow_` after the numeric prefix.
- `090–099` SQL tests: **Club / Member / Identity / Auth** regression tests only, with the same allowed name prefixes.
- `200–229` SQL tests: **School Autopilot / durable workflow orchestration** regression tests only, with `autopilot_` or `workflow_` after the numeric prefix.
