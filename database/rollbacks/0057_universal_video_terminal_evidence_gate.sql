...
-- Removing the terminal evidence gate while queue state exists could make an
-- already-created job finish under weaker rules. Refuse that downgrade.
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM public.schema_migration
         WHERE migration_key = '0057_universal_video_terminal_evidence_gate'
    ) AND EXISTS (SELECT 1 FROM video_queue.job) THEN
        RAISE EXCEPTION 'VIDEO_TERMINAL_EVIDENCE_ROLLBACK_REFUSES_NONEMPTY_QUEUE';
    END IF;
END;
$$;
...
COMMENT ON FUNCTION video_queue.finish_job(uuid,uuid,text,text,jsonb,text) IS NULL;
