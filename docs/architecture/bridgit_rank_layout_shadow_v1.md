...
2. Read job, profile and image files through bounded streams that request at most the applicable limit plus one byte; reject overflow before JSON parsing, hashing or image decode. Pointer arrays, counts and independently checkable event fields are validated before any file access. Compute each image's byte SHA-256 and parse its JPEG/PNG dimensions before calling OpenCV. One decoded BGR raster is capped at 64 MiB and aggregate input rasters at 256 MiB. Before decoding, also reject vertical scan spans above 512 pixels and jobs whose conservative template-call, template-dot-product or recognition-memory estimate exceeds fixed worker budgets. The recognition-memory estimate reserves the complete normalized float32 template bank, its largest construction temporary, and 32 MiB for scoring/allocator workspace in addition to all retained registered frames. Decode once, immediately compare the exact decoded payload's byte SHA-256 with the job declaration before anchor matching or scoring, and compute a decoded-pixel SHA-256. Duplicate bytes, duplicate decoded pixels, or a byte-distinct re-encoding of the reviewed reference pixels cannot increase temporal support.
3. When the verified profile includes `geometry.interface_anchor`, locate its normalized upper-right UI anchor over a bounded list of scales. The zero-mean matcher is invariant to light/dark intensity inversion, derives the translated/scaled game window, and resamples that window to reference geometry. Missing, weak, out-of-frame or ambiguous anchors fail closed; ambiguity compares scale and window size as well as origin. Before any image decode, a whole-job work estimate sums every frame and scale, weights match positions by 256-pixel template blocks, and enforces both per-frame and aggregate ceilings. After bounded payload reads and decode, the aggregate estimate is repeated over the dimensions of those exact payloads before any anchor matching, closing file-replacement races. A separate peak-memory gate counts the reference, all decoded source rasters, all registered output rasters, grayscale buffers, resized templates, correlation arrays and both full-size `CV_64F` integral-image buffers used by `TM_CCOEFF_NORMED`; it rejects the job before matching if their total exceeds 256 MiB. Correlation absolute values are computed in place; catchable Python `MemoryError` and native `cv2.error` failures become `REJECTED` CLI receipts. The receipt retains normalized input-frame anchor/window regions, score, margin, scale, work units and peak scratch estimate. Legacy profiles without this field still require exact reference dimensions and do not establish cross-resolution coverage.
