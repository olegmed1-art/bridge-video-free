...
def audit_stage2_readiness(
    work: Path,
    *,
    expected_total_deals: int | None = None,
    expected_main_tasks: int | None = None,
) -> dict:
    """Audit preparation of the *fresh* main-stage scope, not the pilot corpus.

    Production defaults require the corpus to be expanded to 30,000 deals and
    the shard plan to cover exactly boards 10,001..30,000. Optional expected
    values exist only so tiny deterministic self-tests can exercise the same
    logic without generating 40,000 task rows.
    """
    expected_total_deals = STAGES["main"] if expected_total_deals is None else int(expected_total_deals)
    expected_main_tasks = (
        expected_base_deals("main") * 2
        if expected_main_tasks is None
        else int(expected_main_tasks)
    )
    findings: list[dict] = []
    ...
    corpus_count = int(corpus.get("count", 0) or 0)
    if corpus_count != expected_total_deals:
        add(
            "main_train",
            "MAIN_CORPUS_NOT_EXPANDED",
            f"Stage 2 requires a reproducible corpus of {expected_total_deals} deals; found {corpus_count}.",
        )
    ...
    if not crossfit_tasks:
        add("main_train", "CROSSFIT_TASKS_MISSING", "Generate blind_tasks_crossfit.jsonl after corpus expansion")
        ...
        try:
            ...
            main_tasks = []
            add("main_train", "MAIN_SCOPE_PROVENANCE", str(exc))
        if len(main_tasks) != expected_main_tasks:
            add(
                "main_train",
                "MAIN_FRESH_SCOPE_COVERAGE",
                f"Expected {expected_main_tasks} fresh tasks for boards 10001..30000; found {len(main_tasks)}.",
            )
    ...
    elif int(shard_plan.get("task_count", -1)) != len(main_tasks):
        add("main_train", "SHARD_PLAN_COVERAGE", "Shard plan must cover exactly the fresh main-stage tasks")
    ...
    if not preflight_tasks or not preflight_predictions:
        add("main_train", "LINE_PREFLIGHT_MISSING", "Run a blind legal-line preflight on fresh main-stage tasks")
    else:
        main_ids = {str(task["task_id"]) for task in main_tasks}
        for prediction in preflight_predictions:
            task_id = str(prediction.get("task_id"))
            task = preflight_tasks.get(task_id)
            if task is None:
                add("main_train", "LINE_PREFLIGHT_TASK_MISMATCH", f"No task for prediction {task_id}")
                continue
            if task_id not in main_ids:
                add("main_train", "LINE_PREFLIGHT_NOT_FRESH", f"Preflight task {task_id} is not in the fresh main scope")
                continue
            try:
                validate_prediction_line(task, prediction, require_nonempty=True)
                legal_lines += 1
            except PlayLineError as exc:
                add("main_train", "ILLEGAL_PREFLIGHT_LINE", f"{task_id}: {exc}")
        if legal_lines < min(4, len(preflight_predictions)):
            add("main_train", "INSUFFICIENT_LEGAL_LINES", f"Only {legal_lines} legal fresh-scope preflight lines")
    ...
    # Holdout gate: completed after Stage 2 TRAIN but before validation is opened.
    ...
    # Claim gate: Stage 2 can run without these, but no skill can be called stable.
    ...
    return {
        ...
        "main_train": {
            ...
            "expected_total_deals": expected_total_deals,
            "actual_total_deals": corpus_count,
            "expected_fresh_tasks": expected_main_tasks,
            "actual_fresh_tasks": len(main_tasks),
