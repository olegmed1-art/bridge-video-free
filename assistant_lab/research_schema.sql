...
BEGIN
    IF upper(p_kind) NOT IN ('DDS3','BEN','WORLDS') THEN RAISE EXCEPTION 'only executable DDS3/BEN/WORLDS research jobs may use this function'; END IF;
    IF upper(p_child_kind) NOT IN ('DDS3_COMPUTE','BEN_COMPUTE','WORLD_GENERATE') THEN RAISE EXCEPTION 'invalid child compute kind'; END IF;
