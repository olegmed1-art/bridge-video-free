    ...
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            SELECT
                ...
                (SELECT count(*) FROM video_queue.job bx WHERE bx.batch_id = b.id) AS batch_job_count,
                (
                    SELECT count(*)
                    FROM video_queue.job q
                    JOIN video_queue.batch qb ON qb.id = q.batch_id
            ...
            JOIN video_queue.batch b ON b.id = j.batch_id
            WHERE j.id = %s::uuid
            """,
...
def _strict_processor(job: Mapping[str, Any]) -> dict[str, Any]:
    ...
    processed = stable_review_processor(dict(job))
    ...
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            SELECT
                ...
                b.status AS batch_status,
                (SELECT count(*) FROM video_queue.job bx WHERE bx.batch_id = b.id) AS batch_job_count,
                (SELECT count(*) FROM video_queue.job bx WHERE bx.batch_id = b.id AND bx.status IN ('QUEUED','RETRY','LEASED')) AS unfinished_count
            FROM video_queue.job j
            JOIN video_queue.batch b ON b.id = j.batch_id
            WHERE j.id = %s::uuid
            """,
...
def main(argv: list[str] | None = None) -> int:
    ...
    from universal_video.neon_worker import NeonWorker, process_one_neon

    outcome = process_one_neon(
        NeonWorker(
            database_url=_dsn(),
            worker_id=f"director-one-canary-{runtime_sha[:12]}",
            processing_profile=args.processing_profile,
            algorithm_revision=args.algorithm_revision,
