...
BEGIN
    ...
    IF v_count <> 8 THEN
        RAISE EXCEPTION 'expected eight known Bridge Video version identities, got %', v_count;
    END IF;

    SELECT algorithm_version_id INTO v_version
      FROM algorithm_version
     WHERE algorithm_id=v_algorithm
       AND version_label='3.1-free-r25.10';
    ...
    IF v_count <> 1 THEN
        RAISE EXCEPTION 'initial asset-location verification evidence expected one row, got %', v_count;
    END IF;
    ...
    SELECT count(*) INTO v_count
     ...
     WHERE asset_location_id=v_location;
    IF v_count <> 2 THEN
        RAISE EXCEPTION 'updated asset-location verification should append evidence, got % rows', v_count;
    END IF;
END $$;
