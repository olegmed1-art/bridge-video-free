...
-- School Autopilot Controller v1.4 — brokered, bounded draft repair.
-- GitHub App credentials remain inside the Preview broker. Oracle may submit
-- only the canonical manifest and retain token-free draft-PR evidence.
...
BEGIN
    ...
    IF p_goal_type NOT IN (
        ...
        'GITHUB_CI_READ_ONLY_V1', 'GITHUB_DRAFT_REPAIR_V1'
    ) THEN
    ...
    IF jsonb_typeof(COALESCE(p_goal_json, '{}'::jsonb)) <> 'object'
       OR octet_length(COALESCE(p_goal_json, '{}'::jsonb)::text) >
          (CASE WHEN p_goal_type = 'GITHUB_DRAFT_REPAIR_V1' THEN 65536 ELSE 8192 END) THEN
    ...
    IF p_goal_type IN ('GITHUB_PR_READ_ONLY_V1', 'GITHUB_CI_READ_ONLY_V1') THEN
        ...
        END IF;
    ELSIF p_goal_type = 'GITHUB_DRAFT_REPAIR_V1' THEN
        SELECT array_agg(key ORDER BY key) INTO draft_keys
          FROM jsonb_object_keys(p_goal_json) AS keys(key);
        IF draft_keys IS DISTINCT FROM ARRAY[
               'action_fingerprint', 'allow_force_push', 'allow_merge',
               'base_branch', 'branch_name',
               'changes', 'expected_base_sha', 'production_mutation',
               'repository', 'require_draft', 'task_key', 'title'
           ]
           OR p_goal_json->>'task_key' IS DISTINCT FROM p_task_key
           OR COALESCE(p_goal_json->>'action_fingerprint', '') !~ '^[0-9a-f]{64}$'
           OR COALESCE(p_goal_json->>'task_key', '') !~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$'
           OR p_goal_json->>'repository' <> 'olegmed1-art/bridge-video-free'
           OR p_goal_json->>'base_branch' <> 'main'
           OR COALESCE(p_goal_json->>'expected_base_sha', '') !~ '^[0-9a-f]{40}$'
           OR COALESCE(p_goal_json->>'branch_name', '') !~ '^autopilot/repair/[0-9a-f]{16}$'
           OR length(COALESCE(p_goal_json->>'title', '')) NOT BETWEEN 19 AND 120
           OR p_goal_json->>'title' NOT LIKE '[Autopilot draft] %'
           OR p_goal_json->'require_draft' IS DISTINCT FROM 'true'::jsonb
           OR p_goal_json->'allow_merge' IS DISTINCT FROM 'false'::jsonb
           OR p_goal_json->'allow_force_push' IS DISTINCT FROM 'false'::jsonb
           OR p_goal_json->'production_mutation' IS DISTINCT FROM 'false'::jsonb
           OR jsonb_typeof(p_goal_json->'changes') <> 'array'
           OR jsonb_array_length(p_goal_json->'changes') NOT BETWEEN 1 AND 3
           OR p_cost_cap_microusd <> 0
           OR EXISTS (
               SELECT 1
                 FROM jsonb_array_elements(p_goal_json->'changes') AS changes(item)
                WHERE jsonb_typeof(item) <> 'object'
                   ...
                   OR item->>'path' !~ '^(oracle_autopilot/[A-Za-z0-9_/-]+\.py|tests/test_oracle_autopilot_[A-Za-z0-9_/-]+\.py|docs/evidence/autopilot/[A-Za-z0-9_.-]+\.md)$'
                   OR jsonb_typeof(item->'operation') <> 'string'
                   OR item->>'operation' NOT IN ('CREATE', 'UPDATE')
                   OR jsonb_typeof(item->'content_utf8') <> 'string'
                   OR octet_length(item->>'content_utf8') NOT BETWEEN 1 AND 16384
                   OR (item->>'operation' = 'CREATE' AND item->'expected_blob_sha' <> 'null'::jsonb)
                   OR (item->>'operation' = 'UPDATE' AND (
                       jsonb_typeof(item->'expected_blob_sha') <> 'string'
                       OR item->>'expected_blob_sha' !~ '^[0-9a-f]{40}$'
                   ))
           )
           OR (SELECT sum(octet_length(item->>'content_utf8'))
                 FROM jsonb_array_elements(p_goal_json->'changes') AS changes(item)) > 32768
           OR (SELECT count(DISTINCT item->>'path')
                 FROM jsonb_array_elements(p_goal_json->'changes') AS changes(item))
              <> jsonb_array_length(p_goal_json->'changes') THEN
            RAISE EXCEPTION 'AUTOPILOT_DRAFT_REPAIR_CONTRACT_INVALID';
        END IF;
    ...
    INSERT INTO autopilot.task (
        ...
        CASE p_goal_type
            ...
            WHEN 'GITHUB_DRAFT_REPAIR_V1' THEN 'github.draft_repair'
        ...
        CASE p_goal_type
            ...
            WHEN 'GITHUB_DRAFT_REPAIR_V1' THEN '["github.draft_repair"]'::jsonb
...
'School Autopilot canonical orchestration state. v1.4 permits one brokered bounded draft repair and no merge or production mutation.';

INSERT INTO public.schema_migration(migration_key)
VALUES ('0303_autopilot_github_draft_repair')
ON CONFLICT DO NOTHING;
