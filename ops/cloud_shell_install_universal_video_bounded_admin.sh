...
# One-time OCI Cloud Shell bootstrap for the Universal Video bounded ocarun admin
# entrypoints only. Assistant Lab state is intentionally out of scope. No arbitrary host/user/command.
...
# Final exact-path audit. No general sudo check is granted to ocarun.
ssh "${SSH_OPTIONS[@]}" "$ORACLE_USER@$ORACLE_HOST" \
  'sudo -u ocarun sudo -n /usr/local/sbin/universal-video-oci-admin audit'

echo ORACLE_UNIVERSAL_VIDEO_BOUNDED_ADMIN_BOOTSTRAP_PASS
