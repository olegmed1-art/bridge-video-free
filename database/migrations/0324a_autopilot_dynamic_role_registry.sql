...
DO $$
BEGIN
 IF NOT EXISTS (SELECT 1 FROM public.schema_migration WHERE migration_key='0324_autopilot_project_planner') THEN
  RAISE EXCEPTION 'AUTOPILOT_DYNAMIC_ROLE_REGISTRY_REQUIRES_0324';
 END IF;
END $$;

CREATE TABLE IF NOT EXISTS autopilot.role_registry (
 role_id text PRIMARY KEY CHECK (role_id ~ '^[A-Z][A-Z0-9_]{0,63}$'),
 enabled boolean NOT NULL DEFAULT true,
 execution_scope text NOT NULL DEFAULT 'REPOSITORY' CHECK (execution_scope IN ('REPOSITORY','READ_ONLY_EXTERNAL','OWNER_GATED')),
...
INSERT INTO autopilot.role_registry(role_id,enabled,execution_scope,can_repair,description) VALUES
 ('AUTOPILOT',true,'REPOSITORY',true,'Autopilot code and control plane'),
 ('BOOKS',true,'REPOSITORY',true,'Book material pipeline'),
 ('DATA',true,'REPOSITORY',true,'Data models, migrations and quality'),
 ('DOCUMENTS',true,'READ_ONLY_EXTERNAL',false,'Documents and retained project artifacts'),
 ('GENERAL',true,'OWNER_GATED',false,'Generic owner-approved tasks'),
 ('INFRA',true,'OWNER_GATED',false,'Infrastructure planning and repository-safe verification'),
 ('INTEGRATIONS',true,'OWNER_GATED',false,'External service integrations and connectors'),
 ('KNOWLEDGE',true,'REPOSITORY',true,'Knowledge and Canon processing'),
 ('OPERATIONS',true,'OWNER_GATED',false,'Operational work requiring explicit authority'),
 ('PLANNING',true,'READ_ONLY_EXTERNAL',false,'Planning, decomposition and queue preparation'),
 ('QA',true,'REPOSITORY',true,'Cross-cutting tests and verification'),
 ('RECOGNIZER',true,'REPOSITORY',true,'Card and deal recognition'),
 ('REPORTING',true,'REPOSITORY',true,'Project reports and retained evidence'),
 ('RESEARCH',true,'READ_ONLY_EXTERNAL',false,'Bounded research and evidence collection'),
 ('SECURITY',true,'REPOSITORY',true,'Repository security analysis and bounded repair'),
 ('SERVER',true,'OWNER_GATED',false,'Server operations requiring explicit owner authority'),
 ('SUPPORT',true,'READ_ONLY_EXTERNAL',false,'Triage and user support tasks'),
 ('VIDEO',true,'REPOSITORY',true,'Video 3.1 analysis'),
 ('VIDEO_QUEUE',true,'REPOSITORY',true,'Video queue and receipt pipeline')
ON CONFLICT (role_id) DO NOTHING;

REVOKE ALL ON TABLE autopilot.role_registry FROM PUBLIC, autopilot_runtime, autopilot_runtime_principal, autopilot_callback;

CREATE OR REPLACE FUNCTION autopilot.role_is_enabled(p_role text)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER
SET search_path=pg_catalog,autopilot
AS $$ SELECT EXISTS (SELECT 1 FROM autopilot.role_registry r WHERE r.role_id=p_role AND r.enabled) $$;
REVOKE ALL ON FUNCTION autopilot.role_is_enabled(text) FROM PUBLIC, autopilot_runtime, autopilot_runtime_principal, autopilot_callback;

ALTER TABLE autopilot.project_work_item DROP CONSTRAINT IF EXISTS project_work_item_role_check;
ALTER TABLE autopilot.role_dispatch_outbox DROP CONSTRAINT IF EXISTS role_dispatch_outbox_role_check;
ALTER TABLE autopilot.project_work_item
 ADD COLUMN IF NOT EXISTS task_kind text NOT NULL DEFAULT 'REPOSITORY_AUDIT',
 ADD COLUMN IF NOT EXISTS objective text NOT NULL DEFAULT 'Audit the exact target head and report a bounded result.',
 ADD COLUMN IF NOT EXISTS task_spec_json jsonb NOT NULL DEFAULT '{}'::jsonb;

DO $$
BEGIN
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='project_work_item_role_registry_fk' AND conrelid='autopilot.project_work_item'::regclass) THEN
  ALTER TABLE autopilot.project_work_item ADD CONSTRAINT project_work_item_role_registry_fk FOREIGN KEY(role) REFERENCES autopilot.role_registry(role_id) DEFERRABLE;
 END IF;
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='role_dispatch_outbox_role_registry_fk' AND conrelid='autopilot.role_dispatch_outbox'::regclass) THEN
  ALTER TABLE autopilot.role_dispatch_outbox ADD CONSTRAINT role_dispatch_outbox_role_registry_fk FOREIGN KEY(role) REFERENCES autopilot.role_registry(role_id) DEFERRABLE;
 END IF;
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='project_work_item_task_kind_check' AND conrelid='autopilot.project_work_item'::regclass) THEN
  ALTER TABLE autopilot.project_work_item ADD CONSTRAINT project_work_item_task_kind_check CHECK (task_kind ~ '^[A-Z][A-Z0-9_]{0,63}$');
 END IF;
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='project_work_item_objective_check' AND conrelid='autopilot.project_work_item'::regclass) THEN
  ALTER TABLE autopilot.project_work_item ADD CONSTRAINT project_work_item_objective_check CHECK (length(objective) BETWEEN 1 AND 1000 AND objective !~ '[[:cntrl:]]');
 END IF;
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='project_work_item_task_spec_check' AND conrelid='autopilot.project_work_item'::regclass) THEN
  ALTER TABLE autopilot.project_work_item ADD CONSTRAINT project_work_item_task_spec_check CHECK (jsonb_typeof(task_spec_json)='object' AND octet_length(task_spec_json::text)<=4096);
 END IF;
END $$;

CREATE OR REPLACE FUNCTION autopilot.enforce_enabled_role()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,autopilot
AS $$
BEGIN
 IF NOT autopilot.role_is_enabled(NEW.role) THEN RAISE EXCEPTION 'AUTOPILOT_ROLE_DISABLED_OR_UNKNOWN'; END IF;
 RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS project_work_item_enabled_role ON autopilot.project_work_item;
CREATE TRIGGER project_work_item_enabled_role BEFORE INSERT OR UPDATE OF role ON autopilot.project_work_item
FOR EACH ROW EXECUTE FUNCTION autopilot.enforce_enabled_role();
REVOKE ALL ON FUNCTION autopilot.enforce_enabled_role() FROM PUBLIC, autopilot_runtime, autopilot_runtime_principal, autopilot_callback;

CREATE OR REPLACE FUNCTION autopilot.accept_role_dispatch_callback(p_delivery_id text, p_payload_fingerprint text, p_signature_verified boolean, p_repository text, p_mailbox_pr integer, p_actor_login text, p_actor_id bigint, p_author_association text, p_app_slug text, p_app_id bigint, p_body jsonb)
...
CREATE OR REPLACE FUNCTION autopilot.create_chatgpt_role_dispatch_task(p_task_key text, p_goal_json jsonb, p_priority integer DEFAULT 20, p_created_by text DEFAULT 'DIRECTOR'::text, p_source text DEFAULT 'CHATGPT'::text)
...
BEGIN
    ...
    IF (SELECT array_agg(key ORDER BY key) FROM jsonb_object_keys(p_goal_json) AS keys(key))
       ...
       OR NOT autopilot.role_is_enabled(COALESCE(p_goal_json->>'role', ''))
       ...
       OR (successor_present_count = 4 AND (
           ...
           OR NOT autopilot.role_is_enabled(COALESCE(p_goal_json->>'successor_role', ''))
...
CREATE OR REPLACE FUNCTION autopilot.create_chatgpt_role_followup_task(p_task_key text, p_goal_json jsonb, p_priority integer DEFAULT 20, p_created_by text DEFAULT 'AUTOPILOT_FAILURE_CONTROLLER'::text, p_source text DEFAULT 'AUTOPILOT_FOLLOWUP'::text)
 RETURNS TABLE(task_id uuid, status text, created boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'autopilot'
...
CREATE OR REPLACE FUNCTION autopilot.get_dispatch_assignment(p_dispatch_id uuid)
 RETURNS TABLE(dispatch_id uuid, task_id uuid, role text, execution_scope text, can_repair boolean, task_kind text, objective text, task_spec_json jsonb)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'autopilot'
AS $function$
 SELECT o.dispatch_id,o.task_id,o.role,r.execution_scope,r.can_repair,
        COALESCE(w.task_kind,'REPOSITORY_AUDIT'),
        COALESCE(w.objective,'Audit the exact target head and report a bounded result.'),
        COALESCE(w.task_spec_json,'{}'::jsonb)
 FROM autopilot.role_dispatch_outbox o
 JOIN autopilot.role_registry r ON r.role_id=o.role AND r.enabled
 LEFT JOIN autopilot.project_work_task m ON m.task_id=o.task_id
 LEFT JOIN autopilot.project_work_item w ON w.work_item_id=m.work_item_id
 WHERE o.dispatch_id=p_dispatch_id
$function$;

CREATE OR REPLACE FUNCTION autopilot.register_project_work_item(p_work_key text, p_role text, p_target_pr integer, p_priority integer DEFAULT 20, p_depends_on_work_key text DEFAULT NULL::text, p_created_by text DEFAULT 'CHATGPT_DIRECTOR'::text, p_source text DEFAULT 'SCHOOL_WORKING_BOARD'::text)
...
BEGIN
    IF p_work_key IS NULL
       ...
       OR p_role IS NULL
       OR NOT autopilot.role_is_enabled(p_role)
...
CREATE OR REPLACE FUNCTION autopilot.register_universal_work_item(p_work_key text, p_role text, p_task_kind text, p_objective text, p_target_pr integer DEFAULT 1150, p_priority integer DEFAULT 20, p_task_spec_json jsonb DEFAULT '{}'::jsonb, p_depends_on_work_key text DEFAULT NULL::text, p_created_by text DEFAULT 'CHATGPT_DIRECTOR'::text, p_source text DEFAULT 'UNIVERSAL_WORK_INTAKE'::text)
 RETURNS TABLE(work_item_id uuid, state text, created boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'autopilot'
...
DECLARE
 ...
 dependency_id uuid;
BEGIN
 IF p_work_key IS NULL OR p_work_key !~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,139}$'
    OR NOT autopilot.role_is_enabled(COALESCE(p_role,''))
    OR p_task_kind IS NULL OR p_task_kind !~ '^[A-Z][A-Z0-9_]{0,63}$'
    OR length(COALESCE(p_objective,'')) NOT BETWEEN 1 AND 1000
    OR p_objective ~ '[[:cntrl:]]'
    OR p_target_pr IS NULL OR p_target_pr NOT BETWEEN 1 AND 1000000
    OR p_priority NOT IN (0,10,20,30)
    OR jsonb_typeof(COALESCE(p_task_spec_json,'null'::jsonb)) <> 'object'
    OR octet_length(p_task_spec_json::text) > 4096
    OR length(COALESCE(p_created_by,'')) NOT BETWEEN 1 AND 256
    OR length(COALESCE(p_source,'')) NOT BETWEEN 1 AND 256 THEN
  RAISE EXCEPTION 'AUTOPILOT_UNIVERSAL_WORK_ITEM_INVALID';
 END IF;
 IF p_depends_on_work_key IS NOT NULL THEN
  IF p_depends_on_work_key=p_work_key THEN RAISE EXCEPTION 'AUTOPILOT_PROJECT_WORK_SELF_DEPENDENCY'; END IF;
  SELECT w.work_item_id INTO dependency_id FROM autopilot.project_work_item w WHERE w.work_key=p_depends_on_work_key;
  IF dependency_id IS NULL THEN RAISE EXCEPTION 'AUTOPILOT_PROJECT_WORK_DEPENDENCY_UNKNOWN'; END IF;
 END IF;
 SELECT * INTO existing FROM autopilot.project_work_item w WHERE w.work_key=p_work_key;
 IF FOUND THEN
  IF existing.role<>p_role OR existing.task_kind<>p_task_kind OR existing.objective<>p_objective
     OR existing.target_pr<>p_target_pr OR existing.priority<>p_priority::smallint
     OR existing.task_spec_json<>p_task_spec_json
     OR existing.depends_on_work_item_id IS DISTINCT FROM dependency_id
 ...
 END IF;
 INSERT INTO autopilot.project_work_item(
  work_key,repository,mailbox_pr,role,target_pr,priority,depends_on_work_item_id,
  task_kind,objective,task_spec_json,created_by,source
 ) VALUES (
  p_work_key,'olegmed1-art/bridge-video-free',1150,p_role,p_target_pr,p_priority::smallint,dependency_id,
  p_task_kind,p_objective,p_task_spec_json,p_created_by,p_source
 ) RETURNING * INTO inserted;
 UPDATE autopilot.project_planner_state SET decision_count=decision_count+1,
  last_decision_code='UNIVERSAL_WORK_REGISTERED',last_work_item_id=inserted.work_item_id,last_decision_at=now()
 WHERE singleton;
 PERFORM pg_notify('autopilot_ready','universal-work-registered');
 RETURN QUERY SELECT inserted.work_item_id,inserted.state,true;
END $function$;
...
REVOKE ALL ON FUNCTION autopilot.register_universal_work_item(text,text,text,text,integer,integer,jsonb,text,text,text) FROM PUBLIC, autopilot_runtime, autopilot_runtime_principal, autopilot_callback;
REVOKE ALL ON FUNCTION autopilot.get_dispatch_assignment(uuid) FROM PUBLIC, autopilot_runtime, autopilot_runtime_principal, autopilot_callback;
