...
def verify_result(
    ...
    *,
    ...
    if "speaker_structure" in profile.stages:
        ...
        expected_speaker_schema = (
            SPEAKER_SCHEMA_V2 if profile.name == TEST_PROFILE_NAME else SPEAKER_SCHEMA_V1
        )
        ...
        if status in positive_speaker_statuses:
            ...
            if profile.name == TEST_PROFILE_NAME and (
    ...
    if max_frames is not None and len(manifest_frames) > max_frames:
        raise ResultConformanceError("keyframe count exceeds cap")
    frame_extraction = manifest.get("frame_extraction")
    if profile.name == TEST_PROFILE_NAME and frame_extraction is None:
        raise ResultConformanceError("3.1-test frame extraction evidence is missing")
    expected_frame_times: list[float] | None = None
    if frame_extraction is not None:
        if not isinstance(frame_extraction, dict) or frame_extraction != {
            "schema": "universal-video-frame-extraction-v2",
            "mode": "FIXED_INTERVAL_DENSE_ALLOWED",
            "interval_seconds": frame_extraction.get("interval_seconds"),
            "planned_frames": len(manifest_frames),
            "extracted_frames": len(manifest_frames),
            "implicit_count_cap_applied": False,
        }:
            raise ResultConformanceError("invalid frame extraction evidence")
        interval_seconds = _exact_int(
            frame_extraction.get("interval_seconds"),
            "frame_extraction.interval_seconds",
            minimum=1,
        )
        if interval_seconds > 3600:
            raise ResultConformanceError("frame extraction interval outside contract")
        planned_times = {0.0, max(0.0, media_duration - 0.5)}
        timestamp = float(interval_seconds)
        while timestamp < media_duration:
            planned_times.add(timestamp)
            timestamp += interval_seconds
        expected_frame_times = [round(value, 3) for value in sorted(planned_times)]
        if len(expected_frame_times) != len(manifest_frames):
            raise ResultConformanceError("frame extraction plan/count mismatch")
    seen_frames: set[str] = set()
    observed_frame_times: list[float] = []
    for frame_index, item in enumerate(manifest_frames):
        if not isinstance(item, dict):
            raise ResultConformanceError("invalid keyframe manifest entry")
        frame_time = _bounded_number(
            item.get("time"),
            f"frame {frame_index} time",
            minimum=0.0,
            maximum=media_duration,
        )
        if observed_frame_times and frame_time <= observed_frame_times[-1]:
            raise ResultConformanceError("keyframe timeline is not strictly ordered")
        observed_frame_times.append(round(frame_time, 3))
    ...
    if expected_frame_times is not None and observed_frame_times != expected_frame_times:
        raise ResultConformanceError("frame extraction timeline mismatch")
