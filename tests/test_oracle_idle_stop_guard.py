...
POWER_AUTO = ROOT / ".github" / "workflows" / "oracle-instance-power-auto.yml"
IDLE_CANDIDATE = ROOT / ".github" / "workflows" / "oracle-instance-idle-candidate.yml"
MANUAL_HOLD = ROOT / ".github" / "workflows" / "oracle-manual-hold.yml"
...
class ClassifierHarness:
    def __init__(self) -> None:
        ...
        self.bindir.mkdir()
        _write_executable(
            self.bindir / "systemctl",
            """#!/bin/sh
if [ "$1" = is-active ] && [ "$2" = assistant-lab.service ]; then
  echo active
  exit 0
fi
if [ "$1" = show ]; then
...
""",
    ...
    def run(
        ...
        *,
        ...
        python_stderr: str = "",
        transient_unit_state: str = "absent",
...
if [ "$1" = show ]; then
  case " $* " in
    *" LoadState "*) [ "{transient_unit_state}" = absent ] && echo not-found || echo loaded ;;
    *" ActiveState "*) echo "{transient_unit_state}" ;;
    *) exit 1 ;;
  esac
  exit 0
fi
...
class OracleIdleClassifierTests(unittest.TestCase):
    ...
    # Legacy Pilot/bulk execution can run directly as a systemd unit without
    # first creating a database row.
    def test_active_direct_mass_or_bulk_unit_is_busy(self) -> None:
        completed = self.harness.run(transient_unit_state="active")
        self.assertIn(
            "ORACLE_IDLE_REASON=transient_workload_unit_active",
            completed.stdout,
        )
        self.assert_state(completed.stdout, "BUSY")

    def test_failed_direct_mass_or_bulk_unit_is_unknown(self) -> None:
        completed = self.harness.run(transient_unit_state="failed")
        self.assertIn(
            "ORACLE_IDLE_REASON=transient_workload_unit_failed",
            completed.stdout,
        )
...
class StaticCoverageAndConsumerTests(unittest.TestCase):
    def test_snapshot_covers_every_required_database_family(self) -> None:
        ...
        # The snapshot intentionally has no kind filter: active DDS3, BEN,
        # WORLD and any future bulk job kind are all counted.
        job_count = sql[
            sql.index("FROM assistant_lab.job") :
            sql.index("FROM assistant_lab.research_job")
        ]
        self.assertNotIn("kind", job_count)

        job_schema = (ROOT / "assistant_lab" / "schema.sql").read_text(
            encoding="utf-8"
        )
        self.assertIn("'DDS3_COMPUTE'", job_schema)
        self.assertIn("'BEN_COMPUTE'", job_schema)
    ...
    def test_every_auxiliary_stop_request_uses_the_guarded_consumer(self) -> None:
        for path in (POWER_AUTO, IDLE_CANDIDATE, MANUAL_HOLD):
            workflow = path.read_text(encoding="utf-8")
            self.assertIn("gh workflow run oracle-instance-power.yml", workflow)
