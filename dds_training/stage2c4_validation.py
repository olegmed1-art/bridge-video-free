...
"""Stage 2C.4 validation runner.

The validation protocol is deliberately fail-closed:
- the Stage 2C.4 continuation candidate and selective gate are fit only from
  immutable TRAIN evidence;
- validation positions are selected deterministically before DDS evaluation;
- validation predictions are locked and hashed before any DDS call;
- validation never updates a learning model;
- sealed-test positions are never evaluated.
"""
...
PROTOCOL = "dds-stage2c4-validation-v1"
GATE_MARGIN = 0.08
GATE_MIN_SUPPORT = 20
CARD_MIN_SUPPORT = 8
SEED = 20260818
...
def current_winner(current: Sequence[Mapping[str, object]], add: tuple[int, tuple[int, str]], trump: int) -> int:
    cards = [(int(x["seat"]), SUITS.index(str(x["card"])[0].upper()), str(x["card"])[1].upper()) for x in current]
...
def train_card_loss_model(
    ...
    for row in train_results:
        ...
        for card_name, score in scores.items():
            ...
            stat[feature][0] += best - score
            stat[feature][1] += 1
            actor = str(task["actor"])
            actor_stat[actor][0] += best - score
...
def deterministic_validation_sources(main_tasks_path: Path, source_total: int) -> tuple[list[dict], dict]:
    ...
    for row in validation_contracts:
        ...
        if len(selected) == source_total:
            break
    if len(selected) != source_total:
        raise ValueError(f"insufficient unique validation contract families: {len(selected)} < {source_total}")
    return selected, {
...
def build_validation_curriculum(sources: Sequence[Mapping[str, object]], per_actor: int, line_cards: int) -> tuple[list[dict], dict]:
    candidates = []
    for source in sources:
        ...
        for item in continuation_tasks_from_line(dict(source), line, provenance="predicted_line"):
            if str(item.get("source_root_split")) != "validation":
                raise ValueError("validation continuation lost source split ownership")
            item["priority"] = 0.0
            item["severity"] = 0.0
...
def prepare(args: argparse.Namespace) -> None:
    ...
    train_families = {str(row.get("root_deal_id") or row.get("deal_id")) for row in train_tasks.values()}
    sources, source_meta = deterministic_validation_sources(Path(args.main_tasks), args.source_total)
    validation_source_families = {str(row.get("root_deal_id") or row.get("deal_id")) for row in sources}
    overlap = sorted(train_families & validation_source_families)
    if overlap:
        raise ValueError(f"TRAIN/validation family leakage: {overlap[:10]}")

    curriculum, curriculum_meta = build_validation_curriculum(sources, args.per_actor, args.line_cards)
    ...
    tasks_path = out / "validation_continuation_tasks.jsonl"
    ...
    (out / "locked_validation_predictions.sha256").write_text(
    ...
    evidence = {
        ...
        "source_selection": {
            "policy": "deterministic SHA-256 order, one validation contract source per family, no DDS outcome",
        ...
        "family_overlap_train_validation": len(overlap),
        ...
        "next_action": "DDS validation evaluation only; sealed remains closed",
    }
    write_json(out / "STAGE2C4_VALIDATION_PREOPEN_EVIDENCE.json", evidence)
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    out = Path(args.out)
    out.mkdir(parents=True, exist_ok=True)
    tasks = {str(row["task_id"]): row for row in read_jsonl(root / "validation_continuation_tasks.jsonl")}
    locked_path = root / "locked_validation_predictions.jsonl"
    locked = read_jsonl(locked_path)
    expected_line = (root / "locked_validation_predictions.sha256").read_text(encoding="utf-8").strip().split()[0]
    observed_digest = sha256_file(locked_path)
    if expected_line != observed_digest:
        raise ValueError("locked validation prediction digest changed before DDS evaluation")
    if len(tasks) != 2000 or len(locked) != 2000:
        raise ValueError("validation scope must be exactly 2000 positions")
    if any(str(task.get("source_root_split")) != "validation" for task in tasks.values()):
        raise ValueError("non-validation continuation entered validation scope")
    ...
    for index, prediction in enumerate(locked, 1):
        ...
        scores = {str(card).upper(): float(score) for card, score in scores.items()}
        if not scores:
            raise ValueError(f"DDS candidate scores missing for validation task {prediction['task_id']}")
        best = max(scores.values())
    ...
    write_jsonl(out / "stage2c4_validation_results.jsonl", result_rows)
    by_actor = {}
    ...
    evidence = {
        ...
        "validation_gate_definition": {
            "defense_hybrid_mean_regret_lt_old": True,
            "defense_hybrid_regret_2plus_lt_old": True,
            "declarer_hybrid_mean_regret_le_old_plus": 0.01,
        },
        "next_gate": "owner_decision_on_sealed" if gate else "return_to_train_new_candidate",
    }
    write_json(out / "STAGE2C4_VALIDATION_EVIDENCE.json", evidence)
    report = (
        "# DDS Stage 2C.4 — Validation\n\n"
        "Validation была открыта по явному решению владельца. Предсказания были зафиксированы и "
        "хэшированы до DDS. Обучение на validation запрещено; sealed-test не открывался.\n\n"
        f"Защита: old mean regret {defense['old']['mean_regret']:.3f}, "
        f"hybrid {defense['hybrid']['mean_regret']:.3f}; "
        f"regret>=2 {defense['old']['regret_2plus']} → {defense['hybrid']['regret_2plus']}.\n\n"
        f"Разыгрывающий: old mean regret {declarer['old']['mean_regret']:.3f}, "
        f"hybrid {declarer['hybrid']['mean_regret']:.3f}; "
        f"regret>=2 {declarer['old']['regret_2plus']} → {declarer['hybrid']['regret_2plus']}.\n\n"
        f"Validation gate: {'PASS' if gate else 'FAIL'}.\n\n"
        f"Следующий gate: `{evidence['next_gate']}`. Автоматическое продвижение запрещено.\n"
    )
    (out / "STAGE2C4_VALIDATION_REPORT_RU.md").write_text(report, encoding="utf-8")
    print(json.dumps(evidence, ensure_ascii=False, indent=2))
