...
def _diarized(label_a: str = "Private Teacher", label_b: str = "Private Student"):
    return [
        {
            **_asr_rows()[0],
...
def test_success_reanonymizes_source_labels_and_preserves_asr(monkeypatch, tmp_path: Path):
    ...
    assert "Private Teacher" not in json.dumps({"rows": rows, "report": report})
    assert "Private Student" not in json.dumps({"rows": rows, "report": report})
...
def test_conformance_rejects_real_person_label(tmp_path: Path):
    job_dir, manifest = _bundle(tmp_path)
    _install_two_speaker_evidence(job_dir, manifest, first_label="Private Teacher")
