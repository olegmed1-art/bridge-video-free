...
AS $$
    SELECT
        ...
        (SELECT count(*) FROM assistant_lab.research_job WHERE stage IN ('QUEUED', 'RUNNING'))::bigint,
...
'Least-privilege read-only snapshot for fail-closed Oracle VM stop decisions. Returns only active queue counts; never mutates work state.';
