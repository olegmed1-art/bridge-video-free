...
BEGIN
 IF NOT EXISTS (
   ...
   RAISE EXCEPTION 'AUTOPILOT_0363_ROLLBACK_MIGRATION_MISSING';
 END IF;
 IF (SELECT count(*) FROM autopilot.migration_0363_function_backup)<>2 THEN
   RAISE EXCEPTION 'AUTOPILOT_0363_ROLLBACK_BACKUP_INCOMPLETE';
 END IF;
 IF EXISTS (
   ...
   RAISE EXCEPTION 'AUTOPILOT_0363_ROLLBACK_ACTIVE_ROLE_WORK_PRESENT';
 END IF;
END $pre$;
...
WHERE migration_key='0363_autopilot_audit_findings_repair_progression';
DROP TABLE autopilot.migration_0363_function_backup;
