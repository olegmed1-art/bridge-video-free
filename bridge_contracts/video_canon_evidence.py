...
_LABELLED_HIDDEN_CARDS = re.compile(
    ...
    r"|(?:(?<!\w)(?:рука|карты))\s+(?:партн[её]ра|соперника|оппонента|противника)\b[^;]*?"
...
_SUIT_LABELLED_HIDDEN_CARDS = re.compile(
    ...
    r"(?:hand|cards)\b|(?:(?<!\w)(?:рука|карты))\s+(?:партн[её]ра|соперника|оппонента|противника)\b"
    r"|(?:^|[^A-Za-z0-9])[NESW]\s*:)[^;]*?"
...
_LABELLED_HAND_TAIL = re.compile(
    ...
    r"|(?:(?<!\w)(?:партн[её]р|соперник|оппонент|противник))\s*[:,;=\-]\s*"
...
_RUSSIAN_HIDDEN_POSSESSION = re.compile(
    ...
    r"(?:туз\w*|корол\w*|дам\w*|валет\w*|десятк\w*|двойк\w*|тройк\w*|четв[её]рк\w*|пят[её]рк\w*|шест[её]рк\w*|сем[её]рк\w*|восьм[её]рк\w*|девятк\w*|10|[AKQJT2-9X])"
    r"(?:\s+(?:пик|черв(?:ей|и|а)?|буб(?:ен|ны|на)?|треф(?:ы|а)?))?|"
    r"(?:пиков\w*|червов\w*|бубнов\w*|трефов\w*)\s+"
    r"(?:туз\w*|корол\w*|дам\w*|валет\w*|десятк\w*|двойк\w*|тройк\w*|четв[её]рк\w*|пят[её]рк\w*|шест[её]рк\w*|сем[её]рк\w*|восьм[её]рк\w*|девятк\w*)|"
...
_RUSSIAN_HIDDEN_ABSENCE = re.compile(
    ...
    r"туз\w*|корол\w*|дам\w*|валет\w*|десятк\w*|двойк\w*|тройк\w*|"
    r"четв[её]рк\w*|пят[её]рк\w*|шест[её]рк\w*|сем[её]рк\w*|"
    r"восьм[её]рк\w*|девятк\w*|(?:10|[AKQJT2-9X])(?:\s*[SHDC]\s*:?)?)"
    r"(?=$|[^\w])",
    re.IGNORECASE,
)
...
_LEADING_LENGTH_DESCRIPTION = re.compile(
    r"^\s*(?:(?:was|is)\s+|[:,;=\-]\s*)?(?:10|[2-9]|two|three|four|five|six|seven|eight|nine)\s*(?:(?:(?:[-–—]|to)\s*(?:\d{1,2}|two|three|four|five|six|seven|eight|nine)|\+)\s*)?(?:"
...
_LEADING_NUMERIC_RANGE = re.compile(
    r"^\s*(?:(?:was|is)\s+|[:,;=\-]\s*)?(?:10|[2-9]|two|three|four|five|six|seven|eight|nine)\s*"
    r"(?:(?:[-–—]|to)\s*(?:\d{1,2}|two|three|four|five|six|seven|eight|nine)|\+)(?:$|[^\w])",
    re.IGNORECASE,
)
