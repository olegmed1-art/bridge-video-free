...
def _wrap_injected_parser(parser: LegacyParserInjection):
    ...
    def detector(frame: Path) -> dict[str, Any]:
        ...
        return {"hands": hands, "confidence": confidence, "evidence": {
            "adapter": "explicit-injected-parser", "parser_status": status,
            "recognized_card_count": raw.get("recognized_card_count", count),
            "state_fingerprint": raw.get("state_fingerprint")}}
    return detector


def build_engine(*, allow_legacy_old_bbo: bool = False, allow_ocr_card_labels: bool = False) -> BridgeVisionEngine:
    ...
    if allow_ocr_card_labels:
        from bridge_vision.native_cards import NativeFourSeatCardDetector
        from bridge_vision.ocr_cards import OcrCardLabelBackend
        engine.register("ocr-card-labels", NativeFourSeatCardDetector(OcrCardLabelBackend()))
    return engine


def process_job_frames(job_dir: Path, *, engine: BridgeVisionEngine | None = None,
                       parser: LegacyParserInjection | None = None,
                       allow_legacy_old_bbo: bool = False,
                       allow_ocr_card_labels: bool = False) -> dict[str, Any]:
    ...
    if parser is not None:
        ...
        vision = engine or build_engine(allow_legacy_old_bbo=allow_legacy_old_bbo,
                                        allow_ocr_card_labels=allow_ocr_card_labels)
    records, recognized_frames, conflict_frames = [], 0, 0
    for frame_meta in frames:
        ...
        if compatibility_mode:
            ...
            result["recognized_card_count"] = evidence.get("recognized_card_count",
        ...
        result["time"], result["frame_file"], result["frame_sha256"] = frame_meta.get("time"), frame.name, frame_meta.get("sha256")
        records.append(result)
        recognized_frames += result["deal"] is not None
        conflict_frames += result["status"] == "CONFLICT"
    output_path = root / "bridge_positions.jsonl"
    output_path.write_text("".join(json.dumps(record, ensure_ascii=False, separators=(",", ":")) + "\n" for record in records), encoding="utf-8")
    summary = {"status": "REVIEW" if conflict_frames else "COMPLETED",
        "vision_engine": "native", "detectors": list(vision.detector_names),
        "legacy_old_bbo_enabled": bool(allow_legacy_old_bbo),
        "ocr_card_labels_enabled": bool(allow_ocr_card_labels),
        "job_id": manifest.get("job_id"), "source_fingerprint": manifest.get("source_fingerprint"),
        "input_frames": len(frames), "output_records": len(records),
        "recognized_frames": recognized_frames, "conflict_frames": conflict_frames, "output": output_path.name}
    if compatibility_mode:
        summary["derive_fourth_hand"] = False
    (root / "bridge_positions_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    return summary
...
def main() -> None:
    ...
    parser.add_argument("--allow-legacy-old-bbo", action="store_true",
                        help="explicitly enable the old layout-specific BBO compatibility parser")
    parser.add_argument("--allow-ocr-card-labels", action="store_true",
                        help="explicitly enable OCR card-label recognition; outputs remain review evidence")
    args = parser.parse_args()
    print(json.dumps(process_job_frames(args.job_dir, allow_legacy_old_bbo=args.allow_legacy_old_bbo,
        allow_ocr_card_labels=args.allow_ocr_card_labels), ensure_ascii=False, sort_keys=True))
