...
The backend intentionally recognises only an explicit rank+suit token (``AS``,
``10H``, ``Q♣``).  A rank without a suit, a suit without a rank, and an OCR
guess below the threshold are evidence of *nothing*, not a partly guessed
card.  Seat assignment remains the responsibility of ``native_cards``.

It is useful for lesson recordings that show a bridge diagram or a screen
share.  It is not activated by default: it first has to pass the immutable
gold-frame gate used by :mod:`bridge_vision.pixel_backend`.
"""
...
def tesseract_token_runner(frame: Path) -> tuple[int, int, list[Mapping[str, Any]]]:
    """Read OCR token boxes lazily, so importing the contract needs no OCR SDK."""
    try:
    ...
    for index, text in enumerate(raw.get("text", [])):
        tokens.append({"text": text, "confidence": raw.get("conf", [])[index],
                       "x": raw.get("left", [])[index], "y": raw.get("top", [])[index],
                       "w": raw.get("width", [])[index], "h": raw.get("height", [])[index]})
    return width, height, tokens


class OcrCardLabelBackend:
    """Convert high-confidence OCR card labels into native-card observations."""
    ...
    def __call__(self, frame: Path) -> Mapping[str, Any]:
        ...
        for index, token in enumerate(tokens):
            ...
            try:
                ...
                x, y, w, h = (float(token[key]) for key in ("x", "y", "w", "h"))
            ...
            if w <= 0 or h <= 0:
                ...
                cards.append({"card": card, "confidence": confidence / 100.0,
                              "box": {"x": x, "y": y, "w": w, "h": h}})
        return {"table_region": {"x": 0, "y": 0, "w": float(width), "h": float(height)},
                "cards": cards,
                "ocr_evidence": {"backend_version": OCR_CARD_BACKEND_VERSION, "token_count": len(tokens),
                                 "accepted_card_token_count": len(cards), "rejected_card_tokens": rejected,
