"""Shared bounded Assistant Lab contracts used by web and compute runtimes."""
...
def _validate_ben_payload(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(auction, list) or len(auction) > 80 or any(
        not isinstance(call, str) or len(call) > 16 for call in auction
    ):
