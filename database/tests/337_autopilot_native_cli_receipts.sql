...
BEGIN
 ...
 PERFORM autopilot.mark_role_dispatch_published(dispatch.dispatch_id,label,published.claim_epoch,9900337,repeat('b',64));
...
BEGIN
 ...
 f:=pg_temp.native_fixture('native-sql-success-337'); id:=(f->>'id')::uuid; work_id:=(f->>'work_id')::uuid;
 ...
 r:=autopilot.native_cli_ack(req,'task_e_sql337_success',repeat('c',64));
 IF r IS DISTINCT FROM autopilot.native_cli_ack(req,'task_e_sql337_success',repeat('c',64)) THEN
 ...
 FOR key IN SELECT jsonb_object_keys(result) LOOP
  PERFORM pg_temp.native_reject(format('SELECT autopilot.native_cli_finish(%L,%L,%L)',req,'task_e_sql337_success',result-key),
   'NATIVE_TERMINAL_INVALID');
  PERFORM pg_temp.native_reject(format('SELECT autopilot.native_cli_finish(%L,%L,%L)',req,'task_e_sql337_success',
   jsonb_set(result,ARRAY[key],'null'::jsonb)),'NATIVE_TERMINAL_INVALID');
 END LOOP;
 UPDATE autopilot.project_work_item SET state='PAUSED' WHERE work_item_id=work_id;
 PERFORM pg_temp.native_reject(format('SELECT autopilot.native_cli_finish(%L,%L,%L)',req,'task_e_sql337_success',result),
 ...
 after_result:=autopilot.native_cli_finish(req,'task_e_sql337_success',result);
 ...
 END IF;
 PERFORM autopilot.native_cli_finish(req,'task_e_sql337_success',result);
 SELECT count(*) INTO evidence_count FROM autopilot.evidence WHERE external_ref='codex-cli:task_e_sql337_success';
 ...
 UPDATE autopilot.native_cli_config SET enabled=true;
 f:=pg_temp.native_fixture('native-sql-paused-337'); id:=(f->>'id')::uuid; work_id:=(f->>'work_id')::uuid;
 ...
 PERFORM autopilot.native_cli_ack(req,'task_e_sql337_paused',repeat('c',64));
 result:=result||'{"status":"BLOCKED","result_code":"TARGET_AUTHORITY_CHANGED"}'::jsonb;
 PERFORM autopilot.native_cli_finish(req,'task_e_sql337_paused',result);
 ...
 END IF;
 UPDATE autopilot.role_registry SET enabled=true WHERE role_id='AUTOPILOT';
 f:=pg_temp.native_fixture('native-sql-disabled-role-337'); id:=(f->>'id')::uuid; work_id:=(f->>'work_id')::uuid;
 r:=autopilot.native_cli_reserve(id,f->'assignment','codex/native-disabled-role'); req:=r->'request';
 IF NOT autopilot.native_cli_begin(req) THEN RAISE EXCEPTION 'TEST_NATIVE_BEGIN_DENIED'; END IF;
 PERFORM autopilot.native_cli_ack(req,'task_e_sql337_disabled',repeat('c',64));
 UPDATE autopilot.role_registry SET enabled=false WHERE role_id='AUTOPILOT';
 PERFORM autopilot.native_cli_finish(req,'task_e_sql337_disabled',result);
 ...
 END IF;
 UPDATE autopilot.role_registry SET enabled=true WHERE role_id='AUTOPILOT';
 f:=pg_temp.native_fixture('native-sql-changed-scope-337'); id:=(f->>'id')::uuid; work_id:=(f->>'work_id')::uuid;
 ...
 PERFORM autopilot.native_cli_ack(req,'task_e_sql337_changed',repeat('c',64));
 ...
 PERFORM autopilot.native_cli_finish(req,'task_e_sql337_changed',result);
 IF (SELECT state FROM autopilot.project_work_item WHERE work_item_id=work_id)<>'BLOCKED' THEN
  RAISE EXCEPTION 'TEST_NATIVE_CHANGED_ASSIGNMENT_STRANDED_ACTIVE';
 END IF;
 f:=pg_temp.native_fixture('native-sql-normal-blocked-337'); id:=(f->>'id')::uuid; work_id:=(f->>'work_id')::uuid;
 r:=autopilot.native_cli_reserve(id,f->'assignment','codex/native-normal-blocked'); req:=r->'request';
 IF NOT autopilot.native_cli_begin(req) THEN RAISE EXCEPTION 'TEST_NATIVE_BEGIN_DENIED'; END IF;
 PERFORM autopilot.native_cli_ack(req,'task_e_sql337_blocked',repeat('c',64));
 result:=result||'{"result_code":"BOUNDED_DEFECT","summary":"Synthetic bounded defect."}'::jsonb;
 PERFORM autopilot.native_cli_finish(req,'task_e_sql337_blocked',result);
