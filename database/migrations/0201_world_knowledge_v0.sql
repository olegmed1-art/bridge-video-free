       ...
       SELECT 1
         FROM (
           SELECT step,ord,
                  ...
                  lag(step->>'output_hash') OVER (ORDER BY ord) AS previous_output_hash
        ...
        WHERE (step->>'seq')::numeric <> ord
           ...
           OR (previous_output_hash IS NOT NULL AND step->>'input_hash' IS DISTINCT FROM previous_output_hash)
    )
    OR NEW.decision_trace->'steps'->0->>'input_hash' IS DISTINCT FROM NEW.decision_trace->>'input_fingerprint'
    OR NEW.decision_trace->'steps'->-1->>'output_hash' IS DISTINCT FROM NEW.decision_trace->>'raw_response_sha256'
