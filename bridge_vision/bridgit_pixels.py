...
class PanelStart:
    x:int; y:int; exposed_width:int; exposed_height:int

def _is_card_white(rgb:Sequence[int],*,floor:int=238,spread:int=12)->bool:
    if len(rgb)<3:return False
    r,g,b=(int(rgb[i]) for i in range(3)); return min(r,g,b)>=floor and max(r,g,b)-min(r,g,b)<=spread
...
def exposed_starts_from_scanlines(rows,*,y0=0,min_width=8,min_support=2,merge_tolerance=3):
    ...
    for offset,row in enumerate(rows):
        for start,end in white_runs(row,min_width=min_width):
            ...
            else:match['last_y']=y0+offset;match['support']+=1;match['max_width']=max(match['max_width'],end-start)
    return [PanelStart(c['x'],c['first_y'],c['max_width'],c['last_y']-c['first_y']+1) for c in clusters if c['support']>=min_support]

def persistent_dark_edges(columns:Sequence[Sequence[Sequence[int]]],*,x0:int=0,dark_ceiling:int=60,min_fraction:float=.90,merge_width:int=2)->list[int]:
