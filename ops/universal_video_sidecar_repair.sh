...
fail() {
  local code="$1"
  case "$code" in
    MUST_RUN_AS_ROOT|PROTECTED_SERVICE|DDS3|RUNTIME_LAYOUT|RUNTIME_PIN|RUNTIME_DIRTY|QUEUED_OR_RUNNING_JOB|IMPORT_GATE|SPOOL_REPAIR|SERVICE_START|POST_REGRESSION) ;;
    *) code='RUNTIME_LAYOUT' ;;
  esac
...
runuser -u universal-video -- /usr/bin/env -i \
  ...
  PYTHONPATH="$SOURCE_DIR" HF_HOME="$BASE_DIR/model-cache" \
  "$PYTHON" - <<'PY' >/dev/null || fail IMPORT_GATE
