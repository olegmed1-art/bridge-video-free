...
# One-time root bootstrap for six exact Diana 11 operator commands.
...
grep -Fq "readonly TRANSCRIPT_JOB_HASH='a7253067339ca1ed366279d55584d9eaec5e4cd8d2e0e04c57b9c3b69392785b'" "$SOURCE_FILE" || fail 'unexpected transcript job hash'
grep -Fq "readonly BRIDGE_JOB_ID='diana11-bridge-20260825-01'" "$SOURCE_FILE" || fail 'unexpected bridge job id'
grep -Fq "readonly BRIDGE_JOB_HASH='c80f34c4018c0861c5ba85d9ab0efac63e84027eca755783d15206d416f2d7f6'" "$SOURCE_FILE" || fail 'unexpected bridge job hash'
...
expected_operator_blob="$(git -C "$RUNTIME_SOURCE_DIR" rev-parse "$EXPECTED_RUNTIME_COMMIT:ops/universal_video_diana11_operator.sh")"
...
ocarun ALL=(root) NOPASSWD: /usr/local/sbin/universal-video-diana11 conform-bridge
ocarun ALL=(root) NOPASSWD: /usr/local/sbin/universal-video-diana11 publish-bridge
EOF
chmod 0440 "$tmp"
...
# Exercise both exact read-only status paths after the protected control dirs exist.
bash "$SOURCE_FILE" status >/dev/null
