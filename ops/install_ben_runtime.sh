...
ExecStart=/usr/bin/docker run --rm --name bridge-ben --pull=never --read-only --cap-drop=ALL --security-opt=no-new-privileges:true --pids-limit=256 --memory=6g --memory-swap=6g --cpus=2.0 --shm-size=512m --tmpfs /tmp:rw,nosuid,nodev,noexec,size=1g --tmpfs /logs:rw,nosuid,nodev,noexec,size=64m --tmpfs /app/logs:rw,nosuid,nodev,noexec,size=64m -p 127.0.0.1:8085:8085 $BEN_IMAGE
