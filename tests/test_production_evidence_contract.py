#!/usr/bin/env python3
"""Permanent production guard for the profiled-evidence 3.1 FREE r25.18 route.

r25.18 remains a thin descendant of r25.17 and adds only an opt-in,
human-verified template producer for source-bound SHADOW deal evidence.
Identity, privacy, methodology and zero-paid-AI guards remain intact.
"""
...
import bridge_runtime_hardening_r25_18 as runtime
from bridge_output_scoped_idempotency import existing_same_revision_done
import check_completed_job as preflight
import run_master_3_1_free as base


def test_production_route_is_confirmed_r25_18_with_inheritance_chain():
    ...
    runtime_source = Path("bridge_runtime_hardening_r25_18.py").read_text(encoding="utf-8")
    r25_17_source = Path("bridge_runtime_hardening_r25_17.py").read_text(encoding="utf-8")
    r25_16_source = Path("bridge_runtime_hardening_r25_16.py").read_text(encoding="utf-8")
    ...
    assert "bridge_runtime_hardening_r25_18" in adapter
    assert 'BRIDGE_REQUESTED_ALGORITHM_REVISION: "3.1-free-r25.18"' in workflow
    ...
    assert "import bridge_runtime_hardening_r25_17 as previous" in runtime_source
    ...
    assert "import bridge_runtime_hardening_r25_16 as previous" in r25_17_source
    assert "previous.install(token_func)" in r25_17_source
    assert "import bridge_runtime_hardening_r25_15 as previous" in r25_16_source
    assert "previous.install(token_func)" in r25_16_source
...
def test_deal_evidence_runtime_is_source_bound_shadow_only():
    master = Path("run_master_3_1_free.py").read_text(encoding="utf-8")
    adapter = Path("bridge_vision/deal_evidence.py").read_text(encoding="utf-8")
    pbn = Path("bridge_vision/deal_pbn.py").read_text(encoding="utf-8")

    assert "BRIDGE_DEAL_EVIDENCE_{job}.json" in master
    assert "BRIDGE_VISION_PROFILE.zip" in master
    assert "extract_dense_frames" in master
    assert "produce_deal_evidence_bundle" in master
    assert "apply_deal_evidence_bundle" in master
    assert "render_deals_pbn" in master
    assert "PROFILED_PIXEL_BACKEND" in adapter
    assert "MIN_MACHINE_FRAMES = 2" in adapter
    assert "MIN_MACHINE_CONFIDENCE = 0.90" in adapter
    assert 'RESULT_SCOPE = "SHADOW_ONLY"' in adapter
    assert "canonical_promotion_allowed" in adapter
    assert "production_activation_allowed" in adapter
    assert "X-Observed-" in pbn
    assert "39_TO_13_DECK_SUBTRACTION" in pbn
    assert "canonical_promotion_performed" in pbn
    assert "world_card_backends" not in master
    assert "onnxruntime" not in master
...
if __name__ == "__main__":
    test_production_route_is_confirmed_r25_18_with_inheritance_chain()
    test_deal_evidence_runtime_is_source_bound_shadow_only()
    ...
    print("PRODUCTION_R25_18_EVIDENCE_CONTRACT: PASS")
