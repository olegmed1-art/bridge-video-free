...
def route_position(position_id: UUID) -> dict:
    """Return the next Hybrid Cloud processing tier without doing heavy work inline."""
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            "SELECT * FROM ai.v_position_overview WHERE position_id=%s",
            (position_id,),
        )
        ...
        if position["input_status"] != "COMPLETE":
            return {
                ...
                "route": "INPUT_INCOMPLETE",
                "reason": "position does not contain all mandatory decision inputs",
            }
        ...
        return {
            "position_id": position_id,
            "route": "FAST_REQUIRED",
            "reason": "no cached final decision and no heavy search is currently active",
        }


@router.post("/positions/{position_id}/search-runs")
def enqueue_search(position_id: UUID, request: SearchRequest) -> dict:
    """Queue a heavy search only for a complete position; duplicate active jobs are reused."""
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            "SELECT input_status, scoring FROM ai.decision_position WHERE position_id=%s",
            (position_id,),
        )
        position = cur.fetchone()
        if not position:
        ...
        cur.execute(
            ...
            (
                position_id,
                request.sampler_key,
                request.sampler_version,
                request.rollout_policy,
                request.evaluator_key,
                request.scoring or position["scoring"],
            ),
        )
        search_run = cur.fetchone()
        conn.commit()
        return {"created": True, "search_run": search_run}


@router.post("/search-runs/claim")
def claim_search_run() -> dict:
    """Atomically claim one queued heavy-search job for a serverless worker."""
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            """
            WITH next_job AS (
        ...
        if not search_run:
            conn.commit()
            return {"claimed": False, "search_run": None}
...
def get_search_run(search_run_id: UUID) -> dict:
    with connect() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            """
            SELECT * FROM ai.candidate_evaluation
            WHERE search_run_id=%s
            ORDER BY imp_ev DESC NULLS LAST, raw_score_ev DESC NULLS LAST, created_at
            """,
            (search_run_id,),
        )
...
def complete_search_run(search_run_id: UUID, result: SearchCompletion) -> dict:
    """Persist worker results and make them reusable by all later requests."""
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            "SELECT position_id, status, rollout_policy FROM ai.search_run WHERE search_run_id=%s FOR UPDATE",
            (search_run_id,),
        )
        search_run = cur.fetchone()
        if not search_run:
        ...
        if search_run["status"] == "COMPLETED":
            cur.execute(
                "SELECT * FROM ai.search_run WHERE search_run_id=%s",
                (search_run_id,),
            )
        ...
        if result.status == "COMPLETED":
            for evaluation in result.evaluations:
                cur.execute(
                    "SELECT 1 FROM ai.candidate_action WHERE candidate_id=%s AND position_id=%s",
                    (evaluation.candidate_id, search_run["position_id"]),
                )
                if not cur.fetchone():
                    raise HTTPException(
                        status_code=409,
                        detail=f"candidate {evaluation.candidate_id} does not belong to the search position",
                    )
                rollout_policy = evaluation.rollout_policy or search_run["rollout_policy"]
                cur.execute(
                    """
                    INSERT INTO ai.candidate_evaluation (
                        search_run_id, candidate_id, rollout_policy,
                        raw_score_ev, imp_ev, mp_ev, variance, downside, upside,
                        make_probability, robustness, contracts_json, metrics_json
                    )
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    ON CONFLICT (search_run_id, candidate_id, rollout_policy)
                    DO UPDATE SET
                        raw_score_ev=EXCLUDED.raw_score_ev,
                        imp_ev=EXCLUDED.imp_ev,
                        mp_ev=EXCLUDED.mp_ev,
                        variance=EXCLUDED.variance,
                        downside=EXCLUDED.downside,
                        upside=EXCLUDED.upside,
                        make_probability=EXCLUDED.make_probability,
                    ...
                    (
                        ...
                        Jsonb(evaluation.contracts_json),
                        Jsonb(evaluation.metrics_json),
                    ),
                )
