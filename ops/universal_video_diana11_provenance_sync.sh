...
# submits a job, processes media, writes Drive, or changes DDS3 routing.
...
restore_state(){
  local bad=0
  set +e
  if (( service_touched == 1 )) && systemctl is-active --quiet "$SERVICE"; then
    systemctl stop "$SERVICE" >/dev/null 2>&1 || bad=1
  fi
  if (( env_changed == 1 )); then
    if [[ -n "$backup" && -f "$backup" ]]; then
      cp --preserve=mode,ownership,timestamps "$backup" "$RUNTIME_ENV" || bad=1
      cmp -s "$backup" "$RUNTIME_ENV" || bad=1
    else
      bad=1
    fi
  fi
  if (( service_touched == 1 )); then
    if queue_empty; then
      systemctl start "$SERVICE" >/dev/null 2>&1 || bad=1
      systemctl is-active --quiet "$SERVICE" || bad=1
    else
      # Leave the sidecar stopped rather than process an unexpected job.
      bad=1
    fi
  fi
  return "$bad"
...
if ! RUNTIME_ENV="$RUNTIME_ENV" OUTPUT="$tmp" EXPECTED_REVISION="$EXPECTED_REVISION" python3 - <<'PY' 2>/dev/null
import os
from pathlib import Path
