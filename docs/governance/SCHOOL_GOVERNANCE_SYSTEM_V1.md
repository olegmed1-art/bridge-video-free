# Полная система управления Школой спортивного бриджа — v1.1

Статус: **ACTIVE / CANONICAL**  
Версия: **1.1**
Дата вступления в силу: **2026-08-26**  
Изменение v1.1: **2026-09-03 — AI-verified Video-to-Canon**
...
Решение директора требуется, когда меняется смысл канона вне заранее
авторизованного Video-to-Canon контура или когда автоматические проверки не
могут однозначно разрешить:
...
### 5.4. Автоматическое обучение канона по видеозанятиям

Для видеозанятий доверенного преподавателя поштучное подтверждение человеком
не требуется. AI может автоматически создать и активировать новую версию
SCHOOL CANON, если одновременно выполнены все условия:

- неизменяемая идентичность видео, source SHA-256 и полномочие источника;
- преподаватель и точный фрагмент его речи подтверждены и связаны digest;
- правило, область системы, уровень ученика и период действия структурированы;
- сохранены явные объяснения «почему/зачем» и последствия;
- положительные, отрицательные, граничные и interference-тесты прошли;
- hidden-information firewall прошёл;
- независимые семантическая и бриджевая проверки имеют уровень не ниже I2 и
  выполнены разными verifier families;
- regression, integrity, conflict scan и проверка восстановления прошли;
- отсутствуют ambiguities, открытые противоречия и конфликт активного канона.

Если хотя бы одно условие не доказано, объект остаётся в staging как gap,
conflict или evidence. WORLD не используется для автоматического создания
школьного правила. При `CANON_CONFLICT` WORLD вообще не вызывается.

Автоматическая активация атомарна, версионирована, идемпотентна и сохраняет
полный decision trace. Откат выполняется отзывом новой активации и
восстановлением предыдущей версии без удаления истории. Человек разбирает
только неразрешимые неоднозначности, конфликты и изменение границ доверенных
источников.
...
- каноническое правило: источник + provenance + тесты + AI verification или
  отдельное предметное решение + versioned activation.
...
- первый аудит действующей версии — 2026-09-25;
- полный пересмотр действующей версии — 2026-11-24;
...
- авторизованный Video-to-Canon контур может автоматически изменять канон
  после полного AI evidence gate из §5.4; остальные технические контуры такого
  полномочия не получают.

Изменение v1.1 прошло последовательность `PROPOSAL → IMPACT ANALYSIS → RED
TEAM DESIGN → DIRECTOR APPROVAL → VERSIONED ACTIVATION`. Решение и границы
зафиксированы в
`docs/governance/VIDEO_CANON_AI_PROMOTION_DECISION_2026-09-03.md`.
