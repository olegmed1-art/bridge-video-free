...
The pixel extractor supplies normalized template scores. Colour is only a family
constraint (red => H/D, black => S/C), never a final suit decision. Ambiguous
or weak scores return no suit.
"""
from __future__ import annotations
from typing import Mapping, Any

SUIT_SHAPE_VERSION = "bridge-suit-shape-v1"
_FAMILIES = {"red": ("H", "D"), "black": ("S", "C")}


def classify_suit(scores: Mapping[str, Any], *, colour_family: str, min_score: float = 0.90, min_margin: float = 0.08) -> dict[str, Any]:
    ...
    if not 0 <= min_score <= 1 or not 0 <= min_margin <= 1:
        raise ValueError("invalid suit classifier threshold")
    family = _FAMILIES[colour_family]
    parsed = []
    for suit in family:
        try:
