#!/usr/bin/env python3
"""Regression tests for the r25.8 quality and self-improvement contract."""
import os

import bridge_runtime_hardening_r25_8 as r258


def test_model_contract():
    assert r258.validate_model_contract("medium", "medium") == "medium"
    try:
        r258.validate_model_contract("medium", "small")
...
def test_no_speech_is_explicit_and_conservative():
    ...
    assert r258.normalize_no_speech_qc([silent]) == 1
    ...
    assert r258.normalize_no_speech_qc([spoken]) == 0
    assert not spoken["ok"]


def test_permission_principals_are_removed():
    matrix = r258.principal_free_permission_matrix([
...
def test_hollow_counts_produce_partial_not_false_full():
    ...
    gate = r258.augment_quality_gate(master, {"ok": True})
...
def test_canon_links_are_conservative_and_unique():
    ...
    kept = r258.conservative_canon_links(links, episodes)
...
def test_knowledge_status_never_overclaims():
    ...
    missing = r258.knowledge_status_payload(result, False)
    applied = r258.knowledge_status_payload(result, True)
...
if __name__ == "__main__":
    ...
    print("R25_8_QUALITY_CONTRACT: PASS")
