...
ALTER TABLE autopilot.role_dispatch_followup DROP CONSTRAINT role_dispatch_followup_followup_kind_check;
ALTER TABLE autopilot.role_dispatch_followup ADD CONSTRAINT role_dispatch_followup_followup_kind_check
 CHECK (followup_kind IN ('CONTINUATION','REPAIR','VERIFY','EVIDENCE','RECONCILE'));
...
) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,autopilot AS $$
...
BEGIN
 ...
 SELECT created.task_id INTO followup_id FROM autopilot.create_chatgpt_role_followup_task(
   'role-remediation:'||lower(mode)||':'||origin_row.task_id::text,
   jsonb_build_object('repository',origin_row.goal_json->>'repository','mailbox_pr',1637,
   ...
   origin_row.priority,'AUTOPILOT_REMEDIATION_CONTROLLER','AUTOPILOT_REMEDIATION') created;
