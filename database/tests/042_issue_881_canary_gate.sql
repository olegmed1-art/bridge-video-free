...
BEGIN;
DO $$
DECLARE b record; same record; c record; f record; r record; c2 record; c3 record; bad record; out jsonb; n integer; old uuid;
BEGIN
  ...
  out:=jsonb_build_object('result_mode','SHADOW_REVIEW_ONLY','canonical_promotion_allowed',false,'database_persistence_allowed',false,'publication_state','NOT_PUBLISHED','source_file_id',c.source_file_id,'stable_job_key',c.stable_job_key,'algorithm_revision',c.algorithm_revision,'master_pdf_drive_id','result-file-123456','master_pdf_sha256',repeat('b',64),'artifact_manifest_sha256',repeat('c',64),
    'artifact_manifest',jsonb_build_object('job_id',c.stable_job_key,'source_file_id',c.source_file_id,'result_mode','SHADOW_REVIEW_ONLY','canonical_promotion_allowed',false,'database_persistence_allowed',false,'publication_state','NOT_PUBLISHED','artifacts',jsonb_build_array(jsonb_build_object('kind','master_pdf','locator','gdrive:file:result-file-123456','drive_id','result-file-123456','mime_type','application/pdf','size_bytes',2048,'parent_id','output-folder-123456','sha256',repeat('b',64)))),
    'terminal_receipt',jsonb_build_object('status','PASS','job_id',c.stable_job_key,'source_file_id',c.source_file_id,'drive_readback_verified',true,'artifact_manifest_sha256',repeat('c',64),'canonical_promotion_allowed',false,'database_persistence_allowed',false,'publication_state','NOT_PUBLISHED'));
