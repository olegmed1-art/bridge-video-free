...
def _make_test_bundle(tmp_path: Path) -> tuple[Path, dict]:
    ...
    final_frame = job_dir / "frames" / "frame-final.jpg"
    final_frame.write_bytes(b"jpeg-final")
    manifest["frames"].append(
        {
            "time": 89.5,
            "file": final_frame.name,
            "sha256": hashlib.sha256(final_frame.read_bytes()).hexdigest(),
        }
    )
    manifest["frame_extraction"] = {
...
def test_definition_composes_base_and_all_recent_video_capabilities():
    ...
    assert definition["algorithm_revision"] == "3.1-test-r5"
