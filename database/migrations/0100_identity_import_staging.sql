...
-- Protected staging only. This migration does not create Person, Student,
-- ClubMembership or AuthIdentity records, performs no fuzzy/automatic matching and
-- provides no apply operation. Raw payloads may contain PII and remain owner-only.
...
CREATE INDEX identity_import_batch_state_sequence_idx
...
CREATE TABLE identity_import_item (
    ...
    CHECK (source_payload_hash IS NULL OR btrim(source_payload_hash) <> ''),
...
CREATE INDEX identity_import_item_state_sequence_idx
...
BEGIN
    -- Never trust a caller-provided value for the database evidence hash.
...
-- Current state/action follows immutable append sequence, not a caller-provided timestamp.
...
SELECT DISTINCT ON (i.identity_import_item_id)
    i.identity_import_item_id,i.identity_import_batch_id,i.source_record_key,i.raw_payload_sha256 AS raw_payload_hash,
...
FROM bridge_school_reader,bridge_school_app,bridge_school_worker,bridge_school_health,
     bridge_school_finance,bridge_school_member,bridge_school_member_principal,bridge_school_auth_gateway;
REVOKE ALL ON FUNCTION compute_identity_import_item_hash(),validate_identity_import_batch_scope(),
    validate_identity_import_item_scope(),validate_identity_import_state_scope(),seed_identity_import_batch_state(),
    seed_identity_import_item_state(),validate_identity_import_action(),reject_identity_import_mutation()
FROM PUBLIC,bridge_school_reader,bridge_school_app,bridge_school_worker,bridge_school_health,
     bridge_school_finance,bridge_school_member,bridge_school_member_principal,bridge_school_auth_gateway;

INSERT INTO schema_migration(migration_key) VALUES ('0100_identity_import_staging') ON CONFLICT DO NOTHING;
COMMIT;
