...
import pytest
from ops.validate_universal_video_feature_parity import FeatureParityError, load_and_validate_feature_parity
...
def test_r26_to_oracle_parity_is_honestly_blocked() -> None:
    ...
    assert parity["source_revision"] == "3.1-free-r26.1"
...
def test_oracle_cannot_become_active_while_parity_is_blocked(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    with pytest.raises(RoutingContractError, match="UV_RUNTIME_ROUTE_FEATURE_PARITY_BLOCKED"): load_and_validate()

def test_forged_pass_cannot_hide_unproven_capabilities(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    parity = copy.deepcopy(load_and_validate_feature_parity()); parity["overall_status"] = "PASS"
    ...
    with pytest.raises(FeatureParityError, match="UV_FEATURE_PARITY_STATUS_INCONSISTENT"): load_and_validate_feature_parity()

def test_source_file_cannot_be_used_as_parity_proof(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    parity = copy.deepcopy(load_and_validate_feature_parity()); capability = parity["capabilities"]["deterministic_source_identity"]
    capability["state"] = "PARITY_PROVEN"; capability["proof"] = ["universal_video/runner.py"]; capability["blocker"] = None
    parity_file = tmp_path / "parity.json"; parity_file.write_text(json.dumps(parity), encoding="utf-8")
    monkeypatch.setattr("ops.validate_universal_video_feature_parity.PARITY_FILE", parity_file)
