...
protected_gate_paths=(
  ...
  'ops/native_maintenance_checkpoint_duplex_probe.py'
  ...
  '.github/workflows/autopilot-runtime-role-hardening.yml'
  ...
  '.github/workflows/bridge-video-3.1-server.yml'
  ...
  'ops/run_bridge_video_oracle_legacy_remote.sh'
...
bounded_failure(){
  ...
  abort_database_gate || rc=1
  {
    ...
    grep -E '^UNIVERSAL_VIDEO_(PRECANARY_(WINDOW|RECOVERY|RUNTIME|STATE|STALLED_IDLE_(RESIDENT|RECOVERY)|ATTEST_PASS|FENCED_START|POSTRESTORE_(RUNTIME|OWNER)|OWNER_RELEASE|RESTORE(_PASS|_FAILED)?)|SOURCE_CHECKOUT|CONTAINER_(RESOURCE|STORAGE|CLEANUP|INSTALL_PASS)) |^\{"error_code":"UV_[A-Z0-9_]+","status":"FAILED"\}$|^ERROR:' \
...
grep -F '"checksum_type":"sha256"' "$session_log"
grep -F '"checksum_value":"15c727e42334125caca6c8e506c4ba013eb0b2ba191603f1927fae83dd9cc20b"' "$session_log"
...
trap - ERR
{
  ...
  grep -E '^UNIVERSAL_VIDEO_(PRECANARY_(WINDOW|RECOVERY|RUNTIME|STATE|STALLED_IDLE_(RESIDENT|RECOVERY)|ATTEST_PASS|FENCED_START|POSTRESTORE_(RUNTIME|OWNER)|OWNER_RELEASE|RESTORE_PASS)|SOURCE_CHECKOUT|CONTAINER_(RESOURCE|CLEANUP|INSTALL_PASS)) ' "$session_log"
