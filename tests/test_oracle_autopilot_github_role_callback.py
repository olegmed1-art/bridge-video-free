...
def test_accepts_retained_v2_mailbox_for_late_bound_receipts():
    event = _event()
    event["issue"]["number"] = 1637
    event["issue"]["pull_request"]["url"] = (
        "https://api.github.com/repos/olegmed1-art/bridge-video-free/pulls/1637"
    )
    event["comment"]["issue_url"] = (
        "https://api.github.com/repos/olegmed1-art/bridge-video-free/issues/1637"
    )
    callback = parse_issue_comment_event(event)
