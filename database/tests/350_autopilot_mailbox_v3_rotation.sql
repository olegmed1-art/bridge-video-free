...
BEGIN;
DO $t$
DECLARE active_count integer; allowed text; def text; expected_head text;
BEGIN
 expected_head := CASE
   WHEN EXISTS(SELECT 1 FROM public.schema_migration WHERE migration_key='0354_autopilot_mailbox_v3_final_head')
   THEN '5ff5d9abe497a50cac6564d856297b68c7b4a6c0'
   ELSE '7bfae72289f12ca5c28ce6a3754ccab5383b7f75'
 END;
 ...
 IF NOT EXISTS(SELECT 1 FROM autopilot.role_dispatch_mailbox_registry WHERE mailbox_pr=1685 AND lifecycle='ACTIVE' AND max_dispatches=40 AND expected_head_sha=expected_head) THEN RAISE EXCEPTION 'AUTOPILOT_0350_1685_NOT_ACTIVE'; END IF;
