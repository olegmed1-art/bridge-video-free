...
# Only coarse allowlisted stage/failure markers are consumed by the caller;
...
set_stage 'PREFLIGHT_ASSERTION'
python3 "$SOURCE_FILE" >/dev/null

set_stage 'SUDOERS_TEMPLATE'
tmp="$(mktemp)"
cat > "$tmp" <<'EOF'
