...
def _ocr_compass(image: Any, pytesseract: Any) -> dict[str, tuple[float,float,float]]:
    data = pytesseract.image_to_data(
        image, config="--psm 11 -c tessedit_char_whitelist=NWES",
    ...
    for n,w,e,s in itertools.product(labels["N"], labels["W"], labels["E"], labels["S"]):
        ...
        if not (10 <= span_x <= image.shape[1]*0.35 and 10 <= span_y <= image.shape[0]*0.35):
            continue
        score=(abs(n[0]-s[0])+abs(w[1]-e[1])+
               abs((cy-n[1])-(s[1]-cy))+abs((cx-w[0])-(e[0]-cx))-
               20*min(n[2],w[2],e[2],s[2]))
...
def _rank_tokens(image: Any, pytesseract: Any) -> list[dict[str,Any]]:
    """Character-level rank geometry prevents a suit glyph OCR'd as one digit from
    being fused with the real holding token (for example heart glyph + KQ9 -> 9KQ9)."""
    ...
    for line in boxes.splitlines():
        ...
        if char not in RANKS:
            continue
        try:
            x1=int(fields[1]); y1=int(fields[2]); x2=int(fields[3]); y2=int(fields[4])
        except ValueError:
            continue
        top=height-y2; bottom=height-y1
        tokens.append({
            "text":char, "confidence":0.75,
            "x":x1, "y":top, "right":x2,
            "cx": (x1+x2)/2, "cy": (top+bottom)/2,
        })
    return tokens
