...
# Direct mass/bulk runs predate the database queue and therefore need an
# independent host signal. A loaded active/transitioning unit is BUSY; a
# failed or unreadable unit is UNKNOWN; a not-found unit proves that this
# legacy family is not deployed.
TRANSIENT_WORKLOAD_UNITS="${ORACLE_IDLE_TRANSIENT_WORKLOAD_UNITS:-dds3-mass@10000.service:dds3-mass@30000.service}"
...
if [[ -z "$TRANSIENT_WORKLOAD_UNITS" ]]; then
  mark_unknown "transient_unit_set_missing"
else
  IFS=':' read -r -a transient_units <<< "$TRANSIENT_WORKLOAD_UNITS"
  for unit in "${transient_units[@]}"; do
    [[ -n "$unit" ]] || { mark_unknown "transient_unit_name_empty"; continue; }
    set +e
    load_state="$(systemctl show "$unit" -p LoadState --value 2>/dev/null)"
    load_rc=$?
    set -e
    if (( load_rc != 0 )) || [[ -z "$load_state" ]]; then
      mark_unknown "transient_unit_telemetry_unavailable"
      continue
    fi
    case "$load_state" in
      not-found) continue ;;
      loaded) ;;
      *) mark_unknown "transient_unit_load_state_unknown"; continue ;;
    esac
    set +e
    ...
    case "$active_state" in
      active|activating|deactivating|reloading)
        mark_busy "transient_workload_unit_active"
        ;;
      inactive) ;;
      failed) mark_unknown "transient_workload_unit_failed" ;;
      *) mark_unknown "transient_unit_active_state_unknown" ;;
    esac
  done
fi
