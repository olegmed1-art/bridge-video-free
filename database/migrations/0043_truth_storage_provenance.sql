...
-- -----------------------------------------------------------------------------
-- Truth-layer hardening for automated analysis and storage evidence.
--
-- 1. Register the production Bridge Video analysis algorithm and every version that
--    has already been observed in durable AnalysisRun history.
-- 2. Backfill AnalysisRun.algorithm_version_id and fail closed for future unregistered
--    Bridge Video revisions instead of silently creating provenance-orphan runs.
-- 3. Turn AssetLocation verification state into append-only StorageVerification
--    evidence, including a transparent backfill for locations verified before this
--    migration.
-- -----------------------------------------------------------------------------
...
-- Known versions are registered as observed identities. Registration is not a quality
-- promotion and therefore does not make a version Stable/Operational by itself.
WITH expected(version_no, version_label) AS (
    VALUES
...
-- If production accumulated an additional revision between design and promotion,
-- register that already-observed identity deterministically rather than losing the
-- historical linkage. This does not authorize an unseen future revision.
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Bridge Video algorithm registry conflicts with known observed version identities';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'preexisting Bridge Video analysis run remains unlinked to AlgorithmVersion';
    END IF;
END $$;
...
-- Verification evidence is append-only for runtime workers.
...
VALUES ('0043_truth_storage_provenance')
ON CONFLICT DO NOTHING;
