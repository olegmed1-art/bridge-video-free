...
def test_workflow_accepts_only_two_operations_and_one_instance():
    ...
    assert "remote='sudo -n /usr/local/sbin/universal-video-oci-admin audit 2>&1'" in WORKFLOW
    assert "remote='sudo -n /usr/local/sbin/universal-video-oci-admin productionize 2>&1'" in WORKFLOW
