"""Service-layer helpers for API/chat callers.

This module intentionally contains no HTTP framework coupling. Existing API surfaces
can call these helpers without duplicating queue or validation logic.
"""
...
def submit_research_job(conn: psycopg.Connection, request: dict[str, Any]) -> dict[str, Any]:
    ...
    if priority not in {0, 10, 20, 30}:
        raise ValueError("unsupported research priority")
