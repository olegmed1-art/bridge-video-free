...
-- Controlled staging only: no Person/AuthIdentity/ClubMembership creation and no
-- automatic/fuzzy identity matching. Raw import evidence may contain PII and remains
-- unavailable to runtime/member capabilities.
...
CREATE INDEX identity_import_item_source_identity_idx
    ON identity_import_item(source_identity_id) WHERE source_identity_id IS NOT NULL;
...
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE v_batch_source uuid; v_identity_source uuid;
BEGIN
    SELECT source_id INTO v_batch_source FROM identity_import_batch
     WHERE identity_import_batch_id=NEW.identity_import_batch_id;
    IF v_batch_source IS NULL THEN RAISE EXCEPTION 'identity import item batch missing'; END IF;
    IF NEW.source_identity_id IS NOT NULL THEN
        SELECT source_id INTO v_identity_source FROM source_identity
...
BEGIN
    IF TG_TABLE_NAME='identity_import_batch_state_event' THEN
        IF NOT EXISTS (SELECT 1 FROM identity_import_batch WHERE identity_import_batch_id=NEW.identity_import_batch_id) THEN
            RAISE EXCEPTION 'identity import batch state target missing';
        END IF;
    ELSE
        IF NOT EXISTS (SELECT 1 FROM identity_import_item WHERE identity_import_item_id=NEW.identity_import_item_id) THEN
...
DECLARE
    ...
    v_sup_item uuid;
BEGIN
    SELECT source_identity_id INTO v_item_source_identity FROM identity_import_item
...
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'identity import staging history is append-only'; END;
$$;
CREATE TRIGGER identity_import_batch_immutable BEFORE UPDATE OR DELETE ON identity_import_batch FOR EACH ROW EXECUTE FUNCTION reject_identity_import_mutation();
CREATE TRIGGER identity_import_batch_state_immutable BEFORE UPDATE OR DELETE ON identity_import_batch_state_event FOR EACH ROW EXECUTE FUNCTION reject_identity_import_mutation();
CREATE TRIGGER identity_import_item_immutable BEFORE UPDATE OR DELETE ON identity_import_item FOR EACH ROW EXECUTE FUNCTION reject_identity_import_mutation();
CREATE TRIGGER identity_import_item_state_immutable BEFORE UPDATE OR DELETE ON identity_import_item_state_event FOR EACH ROW EXECUTE FUNCTION reject_identity_import_mutation();
CREATE TRIGGER identity_import_action_immutable BEFORE UPDATE OR DELETE ON identity_import_action FOR EACH ROW EXECUTE FUNCTION reject_identity_import_mutation();
...
FROM identity_import_batch b LEFT JOIN identity_import_batch_state_event se USING(identity_import_batch_id)
ORDER BY b.identity_import_batch_id,se.occurred_at DESC NULLS LAST,se.state_sequence DESC NULLS LAST;

CREATE VIEW identity_import_item_current_state AS
SELECT DISTINCT ON (i.identity_import_item_id)
    i.identity_import_item_id,i.identity_import_batch_id,i.source_record_key,i.raw_payload_hash,
    i.source_identity_id,se.state,se.occurred_at AS state_occurred_at,se.actor_person_id,se.reason,i.created_at
FROM identity_import_item i LEFT JOIN identity_import_item_state_event se USING(identity_import_item_id)
ORDER BY i.identity_import_item_id,se.occurred_at DESC NULLS LAST,se.state_sequence DESC NULLS LAST;
...
SELECT DISTINCT ON (a.identity_import_item_id)
    ...
    a.entity_resolution_decision_id,a.supersedes_action_id,a.actor_person_id,a.reason,a.decided_at,a.created_at
...
SELECT b.identity_import_batch_id,b.school_id,b.source_id,b.external_batch_key,bs.state AS batch_state,
    ...
    count(i.identity_import_item_id) FILTER (WHERE ia.action_type IS NULL)::bigint AS unresolved_count,b.created_at
FROM identity_import_batch b
LEFT JOIN identity_import_batch_current_state bs USING(identity_import_batch_id)
LEFT JOIN identity_import_item i USING(identity_import_batch_id)
LEFT JOIN identity_import_item_current_state isv USING(identity_import_item_id)
LEFT JOIN identity_import_current_action ia USING(identity_import_item_id)
GROUP BY b.identity_import_batch_id,b.school_id,b.source_id,b.external_batch_key,bs.state,b.created_at;

REVOKE ALL ON TABLE identity_import_batch,identity_import_batch_state_event,identity_import_item,
    identity_import_item_state_event,identity_import_action,identity_import_batch_current_state,
    identity_import_item_current_state,identity_import_current_action,identity_import_batch_summary
...
REVOKE ALL ON SEQUENCE identity_import_batch_state_sequence_seq,identity_import_item_state_sequence_seq
FROM bridge_school_reader,bridge_school_app,bridge_school_worker,bridge_school_health,
     bridge_school_finance,bridge_school_member,bridge_school_member_principal,bridge_school_auth_gateway;
REVOKE ALL ON FUNCTION validate_identity_import_batch_scope(),validate_identity_import_item_scope(),
    validate_identity_import_state_scope(),seed_identity_import_batch_state(),seed_identity_import_item_state(),
    validate_identity_import_action(),reject_identity_import_mutation()
FROM PUBLIC,bridge_school_reader,bridge_school_app,bridge_school_worker,bridge_school_health,
     bridge_school_finance,bridge_school_member,bridge_school_member_principal,bridge_school_auth_gateway;

INSERT INTO schema_migration(migration_key) VALUES ('0049_identity_import_staging') ON CONFLICT DO NOTHING;
COMMIT;
