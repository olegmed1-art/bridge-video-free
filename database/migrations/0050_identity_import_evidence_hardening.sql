...
-- Hardening for protected identity-import staging introduced in 0049.
-- Importer-supplied hashes are not trusted evidence; current workflow state/action
-- follows immutable insertion order, not caller-controlled timestamps; staging has no
-- apply operation and therefore cannot claim an applied state.
...
INSERT INTO schema_migration(migration_key) VALUES ('0050_identity_import_evidence_hardening') ON CONFLICT DO NOTHING;
COMMIT;
