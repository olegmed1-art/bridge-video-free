...
def main() -> None:
    ...
    command = [sys.executable, "-c", "print('AUTHORIZED-DRY-RUN')"]
    blocked = run(
        "delegate",
        "--user-approval",
        "ЭТАП-2-ОДОБРЕН",
    ...
    assert "AUTHORIZED-DRY-RUN" not in blocked.stdout
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        prefix = ["--readiness", str(fake), "delegate"]
        no_env = run(*prefix, "--user-approval", "ЭТАП-2-ОДОБРЕН", *command)
