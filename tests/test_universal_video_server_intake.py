...
def test_internal_staging_unlink_failure_is_cleanup_specific(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    spool, staging = tmp_path / "spool", tmp_path / "staging"
    (spool / "inbox").mkdir(parents=True)
    ...
    def fail_unlink(_path: Path, *, missing_ok: bool = False) -> None:
        raise OSError(5, "private staging path")

    monkeypatch.setattr(Path, "unlink", fail_unlink)
    with pytest.raises(IntakeError) as raised:
        submit(payload(), spool_root=spool, staging_root=staging)

    assert raised.value.error_code == "UV_INTAKE_CLEANUP_FAILED"
    assert _error_code(raised.value) == "UV_INTAKE_CLEANUP_FAILED"
    assert "private" not in _error_code(raised.value)
