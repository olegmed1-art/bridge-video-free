...
No production video or video server was used. Seven deterministic synthetic registration cases now exercise a verified upper-right anchor: translated/scaled light UI, translated/scaled intensity-inverted dark UI, absent anchor, two equally plausible game windows, competing scales/window sizes, template-weighted whole-job work overflow, and a file-replacement simulation whose actual decoded dimensions exceed the aggregate budget. Valid cases register to reference geometry; ambiguity and over-budget work fail closed before matching. The profile and receipt use normalized anchor/game-window coordinates and retain bounded work evidence. This proves the registration contract and deterministic implementation, not real-world cross-UI accuracy.
