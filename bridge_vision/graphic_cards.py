    ...
    def __call__(self, frame: Path) -> Mapping[str, Any]:
        ...
        for index, raw in enumerate(candidates):
            ...
            if rank is None or suit is None:
                rejected.append({"index": index, "reason": "INCOMPLETE_OR_INVALID_CARD"})
                continue
            ...
            if rank_conf < self.min_rank_confidence or suit_conf < self.min_suit_confidence:
                rejected.append({"index": index, "card": rank + suit, "reason": "LOW_GRAPHIC_CONFIDENCE", "rank_confidence": rank_conf, "suit_confidence": suit_conf})
                continue
