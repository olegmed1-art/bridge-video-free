...
def rank_move_values(values: list[MoveValue]) -> dict[str, Any]:
    if not values:
        raise ValueError("DDS3 returned no legal moves")
    ...
    return {
        "best_tricks": best,
        "optimal_cards": [r["card"] for r in rows if r["optimal"]],
        "moves": rows,
        "engine": "DDS3",
        "fallback_used": False,
    }
...
def trajectory(values: list[int]) -> dict[str, Any]:
    if not values:
        raise ValueError("trajectory requires at least V0")
    swings = []
    for i in range(1, len(values)):
        delta = values[i] - values[i - 1]
        if delta:
            swings.append({"after_play": i, "from": values[i - 1], "to": values[i], "delta": delta})
    first = swings[0] if swings else None
    first_loss = next((item for item in swings if item["delta"] < 0), None)
    return {
        "values": values,
        "first_swing": first,
        "first_loss": first_loss,
        "final_delta": values[-1] - values[0],
        "unrecovered_damage": max(0, values[0] - values[-1]),
        "swings": swings,
        "engine": "DDS3",
        "fallback_used": False,
    }
