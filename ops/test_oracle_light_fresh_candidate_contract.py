...
def main():
    ...
    assert 'categories.add("DATA")' in restore
    assert 'categories.add("SEQUENCES")' in restore
    assert 'categories.add("SCHEMA_DIGEST")' in restore
    assert "entry_count={len(differing)}" in restore
