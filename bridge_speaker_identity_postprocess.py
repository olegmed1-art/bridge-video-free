...
routed to its isolated output folder.  It never changes raw ASR, the source
video, or anonymous acoustic clusters.  If private identity evidence is
available, it invokes the already field-validated r29 mapping gate.  If no
identity evidence is configured, the transcript remains anonymous and the job
records that named/person-specific attribution is unavailable rather than
...
def _read_json(token: str, item: dict[str, Any]) -> dict[str, Any] | None:
...
def run(token: str) -> dict[str, Any]:
    ...
    evidence_doc_id = os.environ.get("BRIDGE_R29_IDENTITY_EVIDENCE_DOC_ID", "").strip()
    registry_doc_id = os.environ.get("BRIDGE_R29_PARTICIPANT_REGISTRY_DOC_ID", "").strip()

    if len(job_id) != 32 or any(ch not in "0123456789abcdef" for ch in job_id):
        raise RuntimeError("R29_INTEGRATED_INVALID_JOB_ID")
    for name, value in (("OUTPUT", output_folder_id), ("WORK", work_folder_id)):
        if not _valid_drive_id(value):
            raise RuntimeError(f"R29_INTEGRATED_{name}_FOLDER_REQUIRED")
    for value in (evidence_doc_id, registry_doc_id):
    ...
    if not _valid_drive_id(master_pdf_id):
        raise RuntimeError("R29_INTEGRATED_MASTER_PDF_MISSING")

    if bool(evidence_doc_id) != bool(registry_doc_id):
    ...
    if not evidence_doc_id and not registry_doc_id:
        receipt = _status_receipt(
            job_id,
            master_pdf_id,
            "SPEAKER_MAPPING_ANONYMOUS_ONLY",
            "IDENTITY_EVIDENCE_NOT_CONFIGURED",
        )
    ...
    # The following two env values are already set by the workflow/request.  The
    # r29 probe reads private contents at runtime and never logs them.
    r29.main()
    result = {
        "stage": "R29_IDENTITY_OVERLAY",
