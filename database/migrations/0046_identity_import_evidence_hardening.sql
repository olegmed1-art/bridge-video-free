...
-- -----------------------------------------------------------------------------
-- Hardening for the protected identity-import staging layer introduced in 0045.
--
-- 1. A source/importer supplied hash is not trusted as evidence. Preserve it only as
--    optional source metadata and compute a separate SHA-256 inside PostgreSQL.
-- 2. Staging has no apply operation, so the workflow cannot claim an 'applied' state.
-- 3. Current state/action is determined by immutable insertion sequence, not by a
--    caller-supplied timestamp that can be backdated/reordered.
-- 4. Workflow events/actions cannot predate the immutable staged parent record.
-- -----------------------------------------------------------------------------

ALTER TABLE identity_import_item
    RENAME COLUMN raw_payload_hash TO source_payload_hash;
ALTER TABLE identity_import_item
    ALTER COLUMN source_payload_hash DROP NOT NULL;
ALTER TABLE identity_import_item
...
ALTER TABLE identity_import_item
    ALTER COLUMN raw_payload_sha256 SET NOT NULL;
...
DROP TRIGGER IF EXISTS a_identity_import_item_hash_guard ON identity_import_item;
...
-- 0045 intentionally had no apply function; close the corresponding state loophole.
...
ALTER TABLE identity_import_batch_state_event
    VALIDATE CONSTRAINT identity_import_batch_state_no_apply_ck;
ALTER TABLE identity_import_item_state_event
    ADD CONSTRAINT identity_import_item_state_no_apply_ck
    CHECK (state <> 'applied') NOT VALID;
ALTER TABLE identity_import_item_state_event
    VALIDATE CONSTRAINT identity_import_item_state_no_apply_ck;
...
AS $$
BEGIN
    INSERT INTO identity_import_batch_state_event(
        identity_import_batch_id,state,occurred_at,metadata
    ) VALUES (
        NEW.identity_import_batch_id,'staged',NEW.created_at,
        jsonb_build_object('source','batch_insert')
    );
    RETURN NEW;
END;
$$;
...
BEGIN
    INSERT INTO identity_import_item_state_event(
        identity_import_item_id,state,occurred_at,metadata
    ) VALUES (
        NEW.identity_import_item_id,'staged',NEW.created_at,
        jsonb_build_object('source','item_insert')
    );
    RETURN NEW;
END;
$$;
...
-- Preserve the existing view contracts while switching their ordering/evidence source.
...
SELECT DISTINCT ON (i.identity_import_item_id)
    ...
    i.source_record_key,
    i.raw_payload_sha256 AS raw_payload_hash,
...
REVOKE ALL ON FUNCTION compute_identity_import_item_hash()
FROM PUBLIC,bridge_school_reader,bridge_school_app,bridge_school_worker,
     bridge_school_health,bridge_school_finance,bridge_school_member,
     bridge_school_member_principal,bridge_school_auth_gateway;
REVOKE ALL ON FUNCTION validate_identity_import_state_scope(),
    seed_identity_import_batch_state(),seed_identity_import_item_state(),
...
VALUES ('0046_identity_import_evidence_hardening')
