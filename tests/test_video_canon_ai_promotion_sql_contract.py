...
MIGRATION = (ROOT / "database/migrations/0322_video_canon_ai_promotion.sql").read_text()
ROLLBACK = (ROOT / "database/rollbacks/0322_video_canon_ai_promotion.sql").read_text()
...
def test_ai_promotion_is_narrow_guarded_and_not_granted_to_general_workers():
    ...
    assert "GRANT INSERT ON bidding.video_canon_ai_verification TO bridge_school_canon_verifier" in MIGRATION
    assert "bridge_school_app,bridge_school_worker,bridge_school_canon_promoter" in MIGRATION
...
def test_promotion_is_content_bound_idempotent_and_has_fail_closed_rollback():
    ...
    assert "IF v_existing IS NOT NULL THEN RETURN v_existing" in MIGRATION
