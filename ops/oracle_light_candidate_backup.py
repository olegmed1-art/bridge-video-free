...
def restore():
    ...
    with tempfile.TemporaryDirectory(prefix='candidate-drill-',dir=ROOT) as directory:
        ...
        with tarfile.open(DOWNLOADED,'r:gz') as archive:
            ...
            assert len(entries)==len(MEMBERS) and {x.name for x in entries}==MEMBERS
            assert all(x.isfile() and 0<=x.size<16*1024**2 for x in entries)
            assert sum(x.size for x in entries)<32*1024**2
            for entry in entries:
                with archive.extractfile(entry) as source:
                    (work/entry.name).write_bytes(source.read())
        ...
        assert all(digest(work/name)==sha for name,sha in manifest['files'].items())
