...
def _policy_choice(policy: dict[str, Any] | None, candidates: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    if margin is None or margin < POLICY_MIN_MARGIN:
        return None
    ...
    if top_action not in allowed:
        return None
    second = _policy_second_action(policy.get("distribution_json") or {}, top_action)
    return {
        ...
        "evidence": [{
            ...
            "threshold": str(POLICY_MIN_MARGIN),
        }],
    }
...
def finalize_position(position_id: UUID) -> dict:
    """Create a final decision only when the available evidence clears explicit v1 gates."""
    with connect() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            """
            SELECT * FROM ai.candidate_action
            WHERE position_id=%s
            ORDER BY created_at
            """,
            (position_id,),
        ...
        if choice is None:
            cur.execute(
                "SELECT count(*) AS n FROM ai.teacher_output WHERE position_id=%s",
                (position_id,),
            )
            ...
            return {
                ...
                "note": "teacher evidence alone does not authorize a final decision in finalizer-v1",
            }
